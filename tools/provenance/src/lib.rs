...
pub fn validate_bytes(path: &str, bytes: &[u8]) -> ValidationReport {
    ...
    if bytes.len() as u64 <= MAX_RECORD_BYTES {
        ...
        license_policy::augment_bytes(path, bytes, &mut report);
