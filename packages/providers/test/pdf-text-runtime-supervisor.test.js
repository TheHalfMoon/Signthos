...
} = require('../src/pdf/browser/pdf-text-runtime-supervisor');

function bytes() {
  return Buffer.from('%PDF-text-supervisor-test');
}
...
function bindingFor(value = bytes(), overrides = {}) {
  ...
  return {
    ...
    resourceBudgetRef: 'budget:pdf-text-v1:supervisor-test',
    ...overrides,
  };
}
...
async function supervise(overrides = {}) {
  ...
  const runRuntime = overrides.runRuntime ?? (async () => textRaw());
  const runtimeBinding = overrides.runtimeBinding ?? bindingFor(input);
  const promise = supervisePdfTextRuntime({
    bytes: input,
    runtimeBinding,
    runRuntime,
...
test('runtime completion wins without termination and returns a frozen text observation envelope', async () => {
  const raw = textRaw({ pageCount: 3, charCount: 24, text: 'SignthosSignthosSignthos' });
...
test('empty page text observation passes through raw and frozen', async () => {
  const raw = textRaw({ charCount: 0, text: '' });
...
for (const terminalOutcome of [
  ...
  test(`${terminalOutcome} produces exact terminal evidence after confirmed termination`, async () => {
    ...
    runtime.resolve(textRaw({ charCount: 99, text: 'late' }));
...
test('malformed raw text observations fail closed', async () => {
  const cases = [
    null,
    {},
    { openSucceeded: true },
    ...
    { ...textRaw(), truncated: 1 },
    { ...textRaw(), unicodeMapError: 'no' },
    { ...textRaw(), unicodeMapError: 0 },
    { ...textRaw(), truncated: true, charCount: 0, text: '' },
    { ...textRaw(), text: 123 },
    { ...textRaw(), text: null },
    { ...textRaw(), text: Buffer.from('Signthos') },
    { ...textRaw(), pdfiumLastError: 3 },
    { ...textRaw(), extra: true },
    { openSucceeded: false },
  ...
  for (const runtimeBinding of invalidBindings) {
    ...
    await assert.rejects(
      supervisePdfTextRuntime({
        bytes: input,
        runtimeBinding,
        runRuntime: async () => { executions += 1; return textRaw(); },
        terminalControl,
...
test('input and injected objects are not mutated by successful supervision', async () => {
  ...
  const raw = textRaw({ pageCount: 4, charCount: 16, text: 'SignthosSignthos' });
  const terminal = terminalHarness();
  const subscribeBefore = terminal.control.subscribe;
  const result = await supervisePdfTextRuntime({
    bytes: input,
    runtimeBinding,
...
test('text supervisor module exposes only the bounded supervision surface', async () => {
  const supervisor = require('../src/pdf/browser/pdf-text-runtime-supervisor');
  assert.deepEqual(Reflect.ownKeys(supervisor).sort(), [
    'RUNTIME_BINDING_KEYS',
    'SUPERVISOR_KIND',
    'supervisePdfTextRuntime',
  ].sort());
  assert.equal(typeof supervisePdfTextRuntime, 'function');
  assert.equal(Object.isFrozen(supervisor), true);
});

test('text supervisor source contains no real PDFium, WASM execution, provider call, browser/worker or network-loader surface', () => {
  const source = fs.readFileSync(path.join(__dirname, '../src/pdf/browser/pdf-text-runtime-supervisor.js'), 'utf8');
  for (const forbidden of [
    /@embedpdf\/pdfium/,
    /require\('\.\/pdf-text-runtime'\)/,
    /composePdfTextResult\s*\(/,
