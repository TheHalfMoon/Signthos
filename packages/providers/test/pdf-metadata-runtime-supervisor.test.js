...
} = require('../src/pdf/browser/pdf-metadata-runtime-supervisor');

function bytes() {
  return Buffer.from('%PDF-metadata-supervisor-test');
}
...
function bindingFor(value = bytes(), overrides = {}) {
  ...
  return {
    ...
    resourceBudgetRef: 'budget:pdf-metadata-read-v1:supervisor-test',
    ...overrides,
  };
}
...
async function supervise(overrides = {}) {
  ...
  const runRuntime = overrides.runRuntime ?? (async () => metadataRaw());
  const runtimeBinding = overrides.runtimeBinding ?? bindingFor(input);
  const promise = supervisePdfMetadataReadRuntime({
    bytes: input,
    runtimeBinding,
    runRuntime,
...
test('runtime completion wins without termination and returns a frozen metadata observation envelope', async () => {
  const raw = metadataRaw({ metadata: { ...absentMetadata(), title: 'Supervised Title' } });
  ...
  assert.notEqual(result.rawObservation.metadata, raw.metadata);
  assert.equal(Object.isFrozen(result), true);
  assert.equal(Object.isFrozen(result.rawObservation), true);
  assert.equal(Object.isFrozen(result.rawObservation.metadata), true);
...
test('all-absent metadata observation passes through raw and frozen', async () => {
...
for (const terminalOutcome of [
  ...
  test(`${terminalOutcome} produces exact terminal evidence after confirmed termination`, async () => {
    ...
    runtime.resolve(metadataRaw({ metadata: { ...absentMetadata(), title: 'late' } }));
...
test('malformed raw metadata observations fail closed', async () => {
  ...
  const cases = [
    ...
    { ...metadataRaw(), metadata: { ...absentMetadata(), author: {} } },
    { ...metadataRaw(), metadata: { ...absentMetadata(), modDate: ['D:now'] } },
    ...
    { openSucceeded: false, pdfiumLastError: 3, metadata: absentMetadata() },
  ];
  ...
  for (const runtimeBinding of invalidBindings) {
    ...
    await assert.rejects(
      supervisePdfMetadataReadRuntime({
        bytes: input,
        runtimeBinding,
        runRuntime: async () => { executions += 1; return metadataRaw(); },
        terminalControl,
...
test('input and injected objects are not mutated by successful supervision', async () => {
  ...
  const raw = metadataRaw({ metadata: { ...absentMetadata(), producer: 'Kept Producer' } });
  const terminal = terminalHarness();
  const subscribeBefore = terminal.control.subscribe;
  const result = await supervisePdfMetadataReadRuntime({
    bytes: input,
    runtimeBinding,
  ...
  assert.deepEqual(result.rawObservation.metadata.producer, 'Kept Producer');
});

test('metadata supervisor module exposes only the bounded supervision surface', async () => {
  const supervisor = require('../src/pdf/browser/pdf-metadata-runtime-supervisor');
  assert.deepEqual(Reflect.ownKeys(supervisor).sort(), [
    'RUNTIME_BINDING_KEYS',
    'SUPERVISOR_KIND',
    'supervisePdfMetadataReadRuntime',
  ].sort());
  assert.equal(typeof supervisePdfMetadataReadRuntime, 'function');
  assert.equal(Object.isFrozen(supervisor), true);
});

test('metadata supervisor source contains no real PDFium, WASM execution, provider call, browser/worker or network-loader surface', () => {
  const source = fs.readFileSync(path.join(__dirname, '../src/pdf/browser/pdf-metadata-runtime-supervisor.js'), 'utf8');
  for (const forbidden of [
    /@embedpdf\/pdfium/,
    /require\('\.\/pdf-metadata-runtime'\)/,
    /composePdfMetadataResult\s*\(/,
