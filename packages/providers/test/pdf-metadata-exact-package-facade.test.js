...
const {
  readPdfMetadataWithExactPackageSupervisedSemantics,
} = require('../src/pdf/browser/pdf-metadata-exact-package-facade');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `metadata-read:${fixture.fixtureId}:exact-package-facade`,
    ...
    resourceBudgetRef: 'budget:pdf-metadata-read-v1:exact-package-facade-test',
...
test('facade module exposes only the bounded metadata function', () => {
  const facade = require('../src/pdf/browser/pdf-metadata-exact-package-facade');
  assert.deepEqual(Reflect.ownKeys(facade), ['readPdfMetadataWithExactPackageSupervisedSemantics']);
  assert.equal(typeof readPdfMetadataWithExactPackageSupervisedSemantics, 'function');
  assert.equal(Object.isFrozen(facade), true);
});

test('facade source composes only canonical orchestrator and exact package binding', () => {
  const source = fs.readFileSync(
    path.join(root, 'packages/providers/src/pdf/browser/pdf-metadata-exact-package-facade.js'),
    'utf8',
  );
  assert.match(source, /require\('\.\/pdf-metadata-orchestrator'\)/);
  assert.match(source, /require\('\.\/pdf-metadata-runtime-binding'\)/);
  assert.match(source, /orchestratePdfMetadataRead\(/);
  assert.equal(source.match(/readPdfMetadataWithExactPdfiumPackage\(/g).length, 1);
  for (const forbidden of [
    /require\('\.\/pdf-metadata-runtime'\)/,
    /readPdfMetadataWithLocalWasm/,
    /readPdfPageTextWithExactPdfiumPackage\(/,
    /searchPdfPageTextWithExactPdfiumPackage\(/,
    /orchestratePdfMetadata\(/,
    /orchestratePdfTextSearch/,
    /initPdfium/,
    /@embedpdf\/pdfium/,
    /PDFiumExt_Init/,
    /\bFPDF_/,
...
test('facade binds exactly one runRuntime closure over the same caller inputs', () => {
  ...
  assert.match(
    source,
    /const runRuntime = \(\) => readPdfMetadataWithExactPdfiumPackage\(\{\s*bytes,\s*wasmBinary\s*\}\)/,
  );
  assert.match(
    source,
    /orchestratePdfMetadataRead\(\{[\s\S]*?bytes,[\s\S]*?request,[\s\S]*?availability,[\s\S]*?runtimeBinding,[\s\S]*?runRuntime,[\s\S]*?terminalControl,[\s\S]*?terminateRuntime,?[\s\S]*?\}\)/,
  );
});
...
test('top-level proxy options fail without executing proxy traps', async () => {
  ...
  await assert.rejects(
    readPdfMetadataWithExactPackageSupervisedSemantics(proxy),
...
test('top-level accessor, symbol, extra-key, and custom-prototype options fail before terminal subscription', async () => {
  ...
  for (const value of cases) {
    await assert.rejects(
      readPdfMetadataWithExactPackageSupervisedSemantics(value),
      /qualified own data fields/,
    );
  }
...
test('missing wasmBinary key fails before terminal subscription', async () => {
  const counters = {};
  const options = optionsFor({ terminalControl: quietControl(counters) });
  delete options.wasmBinary;
  await assert.rejects(
    readPdfMetadataWithExactPackageSupervisedSemantics(options),
    /qualified own data fields/,
  );
  assert.equal(counters.subscribe || 0, 0);
});
...
test('wasmBinary accessor fails without invoking caller getter code', async () => {
  ...
  await assert.rejects(
    readPdfMetadataWithExactPackageSupervisedSemantics(accessor),
    /qualified own data fields/,
  );
  assert.equal(getterCalls, 0);
  assert.equal(counters.subscribe || 0, 0);
...
test('null-prototype top-level options remain accepted with real package execution', async () => {
  ...
  const result = await readPdfMetadataWithExactPackageSupervisedSemantics(options);
  assert.equal(result.outcome, OUTCOME.SUCCEEDED);
  assert.equal(result.observations.title, null);
});
...
test('ordinary-minimal real package execution returns canonical semantic success with unchanged caller bytes', async () => {
  ...
  const result = await readPdfMetadataWithExactPackageSupervisedSemantics({
    bytes,
    request,
  ...
  assert.equal(result.outcome, OUTCOME.SUCCEEDED);
  assert.deepEqual({ ...result.observations, warnings: undefined }, {
    title: null,
    author: null,
    subject: null,
    keywords: null,
...
for (const [terminalOutcome, errorClass] of [
  ...
  test(`synchronous terminal ${terminalOutcome} wins without PDFium runtime completion and preserves terminal semantics`, async () => {
    ...
    const wasmBefore = Buffer.from(wasmBinary);
    const counters = {};
    const result = await readPdfMetadataWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('terminal callback mutation of caller WASM fails closed', async () => {
  ...
  await assert.rejects(
    readPdfMetadataWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('disposer mutation of caller WASM after runtime completion fails closed without aggregation', async () => {
  ...
  const error = await readPdfMetadataWithExactPackageSupervisedSemantics(optionsFor({
    bytes,
    wasmBinary,
    terminalControl: mutatingControl,
  })).then(
...
test('canonical unavailable and invalid-request semantics remain unchanged through the facade', async () => {
  ...
  const unavailable = await readPdfMetadataWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
    request,
  ...
  const invalidRequest = await readPdfMetadataWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
  ...
  assert.equal(invalidRequest.stableError.errorCode, 'invalid_input.pdf_metadata_read_request');
});
