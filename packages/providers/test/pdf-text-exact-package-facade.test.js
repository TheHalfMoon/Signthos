...
const {
  extractPdfPageTextWithExactPackageSupervisedSemantics,
} = require('../src/pdf/browser/pdf-text-exact-package-facade');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `text:${fixture.fixtureId}:exact-package-facade`,
    ...
    resourceBudgetRef: 'budget:pdf-text-v1:exact-package-facade-test',
...
test('facade module exposes only the bounded text function', () => {
  const facade = require('../src/pdf/browser/pdf-text-exact-package-facade');
  assert.deepEqual(Reflect.ownKeys(facade), ['extractPdfPageTextWithExactPackageSupervisedSemantics']);
  assert.equal(typeof extractPdfPageTextWithExactPackageSupervisedSemantics, 'function');
  assert.equal(Object.isFrozen(facade), true);
});

test('facade source composes only canonical orchestrator and exact package binding', () => {
  const source = fs.readFileSync(
    path.join(root, 'packages/providers/src/pdf/browser/pdf-text-exact-package-facade.js'),
    'utf8',
  );
  assert.match(source, /require\('\.\/pdf-text-orchestrator'\)/);
  assert.match(source, /require\('\.\/pdf-text-runtime-binding'\)/);
  assert.match(source, /orchestratePdfText\(/);
  assert.equal(source.match(/extractPdfPageTextWithExactPdfiumPackage\(/g).length, 1);
...
test('facade binds exactly one runRuntime closure over the same caller inputs', () => {
  ...
  assert.match(
    source,
    /const runRuntime = \(\) => extractPdfPageTextWithExactPdfiumPackage\(\{\s*bytes,\s*wasmBinary,\s*pageIndex,\s*maxChars\s*\}\)/,
  );
  assert.match(
    source,
    /orchestratePdfText\(\{[\s\S]*?bytes,[\s\S]*?request,[\s\S]*?availability,[\s\S]*?runtimeBinding,[\s\S]*?runRuntime,[\s\S]*?terminalControl,[\s\S]*?terminateRuntime,?[\s\S]*?\}\)/,
  );
});
...
test('top-level proxy options fail without executing proxy traps', async () => {
  ...
  await assert.rejects(
    extractPdfPageTextWithExactPackageSupervisedSemantics(proxy),
...
test('top-level accessor, symbol, extra-key, and custom-prototype options fail before terminal subscription', async () => {
  ...
  for (const value of cases) {
    await assert.rejects(
      extractPdfPageTextWithExactPackageSupervisedSemantics(value),
      /qualified own data fields/,
    );
  }
...
test('missing wasmBinary, pageIndex, and maxChars keys fail before terminal subscription', async () => {
  for (const key of ['wasmBinary', 'pageIndex', 'maxChars']) {
    const counters = {};
...
test('wasmBinary accessor fails without invoking caller getter code', async () => {
  ...
  await assert.rejects(
    extractPdfPageTextWithExactPackageSupervisedSemantics(accessor),
    /qualified own data fields/,
  );
  assert.equal(getterCalls, 0);
  assert.equal(counters.subscribe || 0, 0);
...
test('null-prototype top-level options remain accepted with real package execution', async () => {
  ...
  const result = await extractPdfPageTextWithExactPackageSupervisedSemantics(options);
...
test('ordinary-minimal real package execution returns canonical semantic success with unchanged caller bytes', async () => {
  ...
  const result = await extractPdfPageTextWithExactPackageSupervisedSemantics({
    bytes,
    request,
...
for (const [terminalOutcome, errorClass] of [
  ...
  test(`synchronous terminal ${terminalOutcome} wins without PDFium runtime completion and preserves terminal semantics`, async () => {
    ...
    const wasmBefore = Buffer.from(wasmBinary);
    const counters = {};
    const result = await extractPdfPageTextWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('terminal callback mutation of caller WASM fails closed', async () => {
  ...
  await assert.rejects(
    extractPdfPageTextWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('disposer mutation of caller WASM after runtime completion fails closed without aggregation', async () => {
  ...
  const error = await extractPdfPageTextWithExactPackageSupervisedSemantics(optionsFor({
    bytes,
    wasmBinary,
    terminalControl: mutatingControl,
  })).then(
...
test('canonical unavailable and invalid-request semantics remain unchanged through the facade', async () => {
  ...
  const unavailable = await extractPdfPageTextWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
    request,
  ...
  const invalidRequest = await extractPdfPageTextWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
  ...
  assert.equal(invalidRequest.stableError.errorCode, 'invalid_input.pdf_text_request');
});
