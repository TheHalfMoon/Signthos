...
const {
  renderPdfPageWithExactPackageSupervisedSemantics,
} = require('../src/pdf/browser/pdf-render-exact-package-facade');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `render:${fixture.fixtureId}:exact-package-facade`,
    ...
    resourceBudgetRef: 'budget:pdf-render-v1:exact-package-facade-test',
    capabilityParameters: { pageIndex: 0, maxPixels: 1000000 },
    ...overrides,
  };
}
...
function optionsFor(overrides = {}) {
  ...
  return {
    ...
    maxPixels: overrides.maxPixels ?? 1000000,
...
test('facade module exposes only the bounded render function', () => {
  const facade = require('../src/pdf/browser/pdf-render-exact-package-facade');
  assert.deepEqual(Reflect.ownKeys(facade), ['renderPdfPageWithExactPackageSupervisedSemantics']);
  assert.equal(typeof renderPdfPageWithExactPackageSupervisedSemantics, 'function');
  assert.equal(Object.isFrozen(facade), true);
});

test('facade source composes only canonical orchestrator and exact package binding', () => {
  const source = fs.readFileSync(
    path.join(root, 'packages/providers/src/pdf/browser/pdf-render-exact-package-facade.js'),
    'utf8',
  );
  assert.match(source, /require\('\.\/pdf-render-orchestrator'\)/);
  assert.match(source, /require\('\.\/pdf-render-runtime-binding'\)/);
  assert.match(source, /orchestratePdfRender\(/);
  assert.equal(source.match(/renderPdfPageWithExactPdfiumPackage\(/g).length, 1);
...
test('facade binds exactly one runRuntime closure over the same caller inputs', () => {
  ...
  assert.match(
    source,
    /const runRuntime = \(\) => renderPdfPageWithExactPdfiumPackage\(\{\s*bytes,\s*wasmBinary,\s*pageIndex,\s*maxPixels\s*\}\)/,
  );
  assert.match(
    source,
    /orchestratePdfRender\(\{[\s\S]*?bytes,[\s\S]*?request,[\s\S]*?availability,[\s\S]*?runtimeBinding,[\s\S]*?runRuntime,[\s\S]*?terminalControl,[\s\S]*?terminateRuntime,?[\s\S]*?\}\)/,
  );
});
...
test('top-level proxy options fail without executing proxy traps', async () => {
  ...
  await assert.rejects(
    renderPdfPageWithExactPackageSupervisedSemantics(proxy),
...
test('top-level accessor, symbol, extra-key, and custom-prototype options fail before terminal subscription', async () => {
  ...
  for (const value of cases) {
    await assert.rejects(
      renderPdfPageWithExactPackageSupervisedSemantics(value),
      /qualified own data fields/,
    );
  }
...
test('missing wasmBinary, pageIndex, and maxPixels keys fail before terminal subscription', async () => {
  for (const key of ['wasmBinary', 'pageIndex', 'maxPixels']) {
    const counters = {};
    const options = optionsFor({ terminalControl: quietControl(counters) });
    delete options[key];
    await assert.rejects(
      renderPdfPageWithExactPackageSupervisedSemantics(options),
      /qualified own data fields/,
    );
    assert.equal(counters.subscribe || 0, 0);
  }
...
test('wasmBinary accessor fails without invoking caller getter code', async () => {
  ...
  await assert.rejects(
    renderPdfPageWithExactPackageSupervisedSemantics(accessor),
    /qualified own data fields/,
  );
  assert.equal(getterCalls, 0);
  assert.equal(counters.subscribe || 0, 0);
...
test('null-prototype top-level options remain accepted with real package execution', async () => {
  ...
  const result = await renderPdfPageWithExactPackageSupervisedSemantics(options);
...
test('ordinary-minimal real package execution returns canonical semantic success with unchanged caller bytes', async () => {
  ...
  const result = await renderPdfPageWithExactPackageSupervisedSemantics({
    bytes,
    request,
  ...
  assert.equal(result.observations.pageIndex, 0);
  assert.equal(result.observations.pageCount, 1);
  assert.ok(result.observations.width > 0);
  assert.ok(result.observations.height > 0);
  assert.equal(result.observations.pixelDigest.algorithm, 'sha256');
...
for (const [terminalOutcome, errorClass] of [
  ...
  test(`synchronous terminal ${terminalOutcome} wins without PDFium runtime completion and preserves terminal semantics`, async () => {
    ...
    const wasmBefore = Buffer.from(wasmBinary);
    const counters = {};
    const result = await renderPdfPageWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('terminal callback mutation of caller WASM fails closed', async () => {
  ...
  await assert.rejects(
    renderPdfPageWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('disposer mutation of caller WASM after runtime completion fails closed without aggregation', async () => {
  ...
  const error = await renderPdfPageWithExactPackageSupervisedSemantics(optionsFor({
    bytes,
    wasmBinary,
    terminalControl: mutatingControl,
  })).then(
...
test('canonical unavailable and invalid-request semantics remain unchanged through the facade', async () => {
  ...
  const unavailable = await renderPdfPageWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
    request,
  ...
  const invalidRequest = await renderPdfPageWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
  ...
  assert.equal(invalidRequest.stableError.errorCode, 'invalid_input.pdf_render_request');
});
