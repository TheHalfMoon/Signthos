...
const { SUPERVISOR_KIND } = require('../src/pdf/browser/pdf-metadata-runtime-supervisor');
const { composeSupervisedPdfMetadataResult } = require('../src/pdf/browser/pdf-metadata-runtime-bridge');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `metadata-read:${fixture.fixtureId}:bridge`,
    ...
    resourceBudgetRef: 'budget:pdf-metadata-read-v1:bridge-test',
...
function metadataRaw(overrides = {}) {
  return {
    openSucceeded: true,
...
function compose(supervisedResult, overrides = {}) {
  ...
  return composeSupervisedPdfMetadataResult({
    bytes,
    request,
...
test('runtime completion maps canonical all-absent metadata into semantic success with explicit nulls', () => {
  const bytes = bytesForFixture();
  const request = requestFor(bytes);
  const raw = metadataRaw();
  const result = composeSupervisedPdfMetadataResult({
    bytes,
    request,
...
test('runtime completion maps positive metadata into exact semantic observations', () => {
  const result = compose(completed(metadataRaw({
    metadata: { ...absentMetadata(), title: 'Bridge Title', creator: 'Bridge Creator' },
  })));
  assert.equal(result.outcome, OUTCOME.SUCCEEDED);
  assert.equal(result.observations.title, 'Bridge Title');
  assert.equal(result.observations.creator, 'Bridge Creator');
...
test('completed path preserves canonical unavailable semantics without metadata publication', () => {
  const result = compose(completed(metadataRaw()), {
    availability: AVAILABILITY.UNKNOWN,
  });
  assert.equal(result.outcome, OUTCOME.FAILED);
  assert.equal(result.stableError.errorClass, STABLE_ERROR.UNAVAILABLE);
  assert.equal(result.stableError.errorCode, 'unavailable.pdf_metadata_read_provider');
});
...
test('semantic request byte mismatch remains a canonical invalid-input result', () => {
  ...
  const result = composeSupervisedPdfMetadataResult({
    ...
    request,
    availability: AVAILABILITY.AVAILABLE,
    supervisedResult: completed(metadataRaw()),
  });
  assert.equal(result.outcome, OUTCOME.FAILED);
  assert.equal(result.stableError.errorClass, STABLE_ERROR.INVALID_INPUT);
...
test('bridge requires Buffer bytes at the semantic boundary', () => {
  const bytes = bytesForFixture();
  assert.throws(() => composeSupervisedPdfMetadataResult({
...
test('unknown, extra-key, symbol-bearing, and custom-prototype supervisor envelopes fail closed', () => {
  const cases = [
    { kind: 'UNKNOWN', rawObservation: metadataRaw() },
    { kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: metadataRaw(), extra: true },
    Object.assign({ kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: metadataRaw() }, { [Symbol('extra')]: true }),
    Object.assign(Object.create({ inherited: true }), { kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: metadataRaw() }),
  ];
...
test('proxy-backed supervisor envelope is rejected without executing proxy traps', () => {
  let traps = 0;
  const proxy = new Proxy(completed(metadataRaw()), {
...
test('accessor-backed supervisor kind is rejected without invoking the getter', () => {
  let getterCalls = 0;
  const envelope = { rawObservation: metadataRaw() };
...
test('malformed raw metadata observations fail before the provider can traverse unsafe values', () => {
  let proxyTraps = 0;
  const proxyRaw = new Proxy(metadataRaw(), {
    getPrototypeOf() { proxyTraps += 1; throw new Error('trap executed'); },
    ownKeys() { proxyTraps += 1; throw new Error('trap executed'); },
  });

  let getterCalls = 0;
  const accessorRaw = metadataRaw();
  ...
  for (const rawObservation of [
    proxyRaw,
    accessorRaw,
    { ...metadataRaw(), extra: true },
    Object.assign(metadataRaw(), { [Symbol('extra')]: true }),
    Object.assign(Object.create({ inherited: true }), metadataRaw()),
    { ...metadataRaw(), metadata: { ...absentMetadata(), title: 42 } },
    { ...metadataRaw(), metadata: partial },
    { ...metadataRaw(), openSucceeded: true, pdfiumLastError: 3 },
...
test('bridge does not mutate caller bytes, request, or supervised predecessor evidence', () => {
  const bytes = bytesForFixture();
  const request = requestFor(bytes);
  const rawObservation = metadataRaw();
...
test('bridge source contains no runtime, PDFium, browser, timer, network, or dependency execution surface', () => {
  const sourcePath = path.join(root, 'packages/providers/src/pdf/browser/pdf-metadata-runtime-bridge.js');
  const source = fs.readFileSync(sourcePath, 'utf8');
  for (const forbidden of [
    /readPdfMetadataWithLocalWasm/,
    /supervisePdfMetadataReadRuntime\s*\(/,
    /@embedpdf\/pdfium/,
    /PDFiumExt_Init/,
    /\bFPDF_/,
    /\bWebAssembly\b/,
...
test('null-prototype completed envelope and raw observation remain accepted', () => {
  const rawObservation = Object.assign(Object.create(null), metadataRaw());
  ...
  assert.equal(result.outcome, OUTCOME.SUCCEEDED);
  assert.deepEqual(result.observations.title, null);
});
