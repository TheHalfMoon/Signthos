...
} = require('../src/pdf/browser/pdf-text-search-runtime-supervisor');

function bytes() {
  return Buffer.from('%PDF-text-search-supervisor-test');
}
...
function bindingFor(value = bytes(), overrides = {}) {
  ...
  return {
    ...
    resourceBudgetRef: 'budget:pdf-text-search-v1:supervisor-test',
    ...overrides,
  };
}
...
async function supervise(overrides = {}) {
  ...
  const runRuntime = overrides.runRuntime ?? (async () => searchRaw());
  const runtimeBinding = overrides.runtimeBinding ?? bindingFor(input);
  const promise = supervisePdfTextSearchRuntime({
    bytes: input,
    runtimeBinding,
    runRuntime,
...
test('runtime completion wins without termination and returns a frozen selection observation envelope', async () => {
  const raw = searchRaw({ pageCount: 3, charCount: 24, queryLength: 4, matchCount: 2 });
  ...
  assert.deepEqual(result.rawObservation, { ...raw, matches: [match(0, 4), match(4, 4)] });
...
test('empty selection observation passes through raw and frozen', async () => {
  const raw = searchRaw({ charCount: 0, matchCount: 0, matches: [] });
...
for (const terminalOutcome of [
  ...
  test(`${terminalOutcome} produces exact terminal evidence after confirmed termination`, async () => {
    ...
    runtime.resolve(searchRaw({ charCount: 99, queryLength: 99 }));
  ...
  for (const runtimeBinding of invalidBindings) {
    ...
    await assert.rejects(
      supervisePdfTextSearchRuntime({
        bytes: input,
        runtimeBinding,
        runRuntime: async () => { executions += 1; return searchRaw(); },
        terminalControl,
...
test('input and injected objects are not mutated by successful supervision', async () => {
  ...
  const raw = searchRaw({ pageCount: 4, charCount: 16 });
  const terminal = terminalHarness();
  const subscribeBefore = terminal.control.subscribe;
  const result = await supervisePdfTextSearchRuntime({
    bytes: input,
    runtimeBinding,
...
test('select supervisor module exposes only the bounded supervision surface', async () => {
  const supervisor = require('../src/pdf/browser/pdf-text-search-runtime-supervisor');
  assert.deepEqual(Reflect.ownKeys(supervisor).sort(), [
    'RUNTIME_BINDING_KEYS',
    'SUPERVISOR_KIND',
    'supervisePdfTextSearchRuntime',
  ].sort());
  assert.equal(typeof supervisePdfTextSearchRuntime, 'function');
...
test('select supervisor source contains no real PDFium, WASM execution, provider call, browser/worker or network-loader surface', () => {
  const source = fs.readFileSync(path.join(__dirname, '../src/pdf/browser/pdf-text-search-runtime-supervisor.js'), 'utf8');
  for (const forbidden of [
    /@embedpdf\/pdfium/,
    /require\('\.\/pdf-text-search-runtime'\)/,
    /composePdfTextSearchResult\s*\(/,
