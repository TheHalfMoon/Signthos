...
const {
  selectPdfPageTextWithExactPdfiumPackage,
} = require('../src/pdf/browser/pdf-text-select-runtime-binding');

const BINDING_OPTIONS_MESSAGE = 'PDF text selection exact-package runtime binding options must contain only qualified own data fields';
...
function optionsFor(overrides = {}) {
  return {
    ...
    startIndex: 'startIndex' in overrides ? overrides.startIndex : 0,
    selectCount: 'selectCount' in overrides ? overrides.selectCount : 8,
    maxRects: 'maxRects' in overrides ? overrides.maxRects : 4,
    ...Object.fromEntries(Object.entries(overrides).filter(([key]) => !['bytes', 'wasmBinary', 'pageIndex', 'startIndex', 'selectCount', 'maxRects'].includes(key))),
  };
}
...
test('exact package runtime binding exposes only the bounded selection function', () => {
  const binding = require('../src/pdf/browser/pdf-text-select-runtime-binding');
  assert.deepEqual(Reflect.ownKeys(binding), ['selectPdfPageTextWithExactPdfiumPackage']);
  assert.equal(typeof selectPdfPageTextWithExactPdfiumPackage, 'function');
...
test('binding source imports the exact package initializer without asset or network resolution', () => {
  const sourcePath = path.join(
    __dirname,
    '../src/pdf/browser/pdf-text-select-runtime-binding.js',
  );
  const source = fs.readFileSync(sourcePath, 'utf8');

  assert.match(source, /require\('@embedpdf\/pdfium'\)/);
  assert.match(source, /require\('\.\/pdf-text-select-runtime'\)/);
...
test('binding source carries no provider, supervisor, bridge, orchestrator, or cross-track behavior', () => {
  ...
  for (const forbidden of [
    /require\('\.\/pdf-text-select-provider'\)/,
    /require\('\.\/pdf-text-select-runtime-supervisor'\)/,
    /require\('\.\/pdf-text-select-runtime-bridge'\)/,
    /require\('\.\/pdf-text-select-orchestrator'\)/,
    /extractPdfPageTextWithLocalWasm/,
    /orchestratePdfText\(/,
    /composePdfTextSelectResult/,
    /supervisePdfTextSelectRuntime/,
    /composeSupervisedPdfTextSelectResult/,
    /orchestratePdfTextSelect/,
  ]) {
...
test('binding rejects malformed envelope shapes before runtime effects', async () => {
  ...
  for (const input of malformed) {
    await assert.rejects(
      selectPdfPageTextWithExactPdfiumPackage(input),
...
test('binding rejects accessor, extra-key, missing-key, and symbol-key envelopes before runtime effects', async () => {
  ...
  const { maxRects: _dropped, ...withMissingKey } = optionsFor();
...
test('binding preserves canonical byte-view validation without invoking the runtime', async () => {
  ...
  await assert.rejects(
    selectPdfPageTextWithExactPdfiumPackage(optionsFor({ bytes: new Uint8Array(), wasmBinary })),
    /bytes must be a non-empty Uint8Array/,
  );
  await assert.rejects(
    selectPdfPageTextWithExactPdfiumPackage(optionsFor({ bytes, wasmBinary: new Uint8Array() })),
...
test('binding preserves canonical page, range, and rect-budget validation without invoking the runtime', async () => {
  for (const pageIndex of [-1, 1.5, '0', null]) {
    await assert.rejects(
      selectPdfPageTextWithExactPdfiumPackage(optionsFor({ pageIndex })),
      /pageIndex must be a non-negative safe integer/,
    );
  }
  for (const startIndex of [-1, 2.5, '0', null]) {
    await assert.rejects(
      selectPdfPageTextWithExactPdfiumPackage(optionsFor({ startIndex })),
      /startIndex must be a non-negative safe integer/,
    );
  }
  for (const selectCount of [-1, 2.5, '8', null]) {
    await assert.rejects(
      selectPdfPageTextWithExactPdfiumPackage(optionsFor({ selectCount })),
      /selectCount must be a PDFium int character count/,
    );
  }
  for (const maxRects of [0, -4, 2.5, '4', null]) {
    await assert.rejects(
      selectPdfPageTextWithExactPdfiumPackage(optionsFor({ maxRects })),
      /maxRects must be a positive PDFium int/,
    );
  }
});

test('binding delegates package init and raw selection extraction for caller-supplied bytes', async () => {
  const bytes = bytesForFixture();
  const wasmBinary = adoptedWasmBytes();
  const bytesBefore = Uint8Array.from(bytes);
  const wasmBefore = Uint8Array.from(wasmBinary);

  const result = await selectPdfPageTextWithExactPdfiumPackage({
    bytes,
    wasmBinary,
    pageIndex: 0,
    startIndex: 0,
  ...
  assert.equal(result.selectCount, 8);
...
test('mismatched caller WASM bytes fail closed without publishing selection success', async () => {
  const bytes = bytesForFixture();
