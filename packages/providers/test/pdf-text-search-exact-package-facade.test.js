...
const {
  searchPdfPageTextWithExactPackageSupervisedSemantics,
} = require('../src/pdf/browser/pdf-text-search-exact-package-facade');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `text-search:${fixture.fixtureId}:exact-package-facade`,
    ...
    resourceBudgetRef: 'budget:pdf-text-search-v1:exact-package-facade-test',
...
function optionsFor(overrides = {}) {
  ...
  return {
    ...
    query: overrides.query ?? 'Sign',
    maxMatches: overrides.maxMatches ?? 4,
...
test('facade module exposes only the bounded text function', () => {
  const facade = require('../src/pdf/browser/pdf-text-search-exact-package-facade');
  assert.deepEqual(Reflect.ownKeys(facade), ['searchPdfPageTextWithExactPackageSupervisedSemantics']);
  assert.equal(typeof searchPdfPageTextWithExactPackageSupervisedSemantics, 'function');
  assert.equal(Object.isFrozen(facade), true);
});

test('facade source composes only canonical orchestrator and exact package binding', () => {
  const source = fs.readFileSync(
    path.join(root, 'packages/providers/src/pdf/browser/pdf-text-search-exact-package-facade.js'),
    'utf8',
  );
  assert.match(source, /require\('\.\/pdf-text-search-orchestrator'\)/);
  assert.match(source, /require\('\.\/pdf-text-search-runtime-binding'\)/);
  assert.match(source, /orchestratePdfTextSearch\(/);
  assert.equal(source.match(/searchPdfPageTextWithExactPdfiumPackage\(/g).length, 1);
...
test('facade binds exactly one runRuntime closure over the same caller inputs', () => {
  ...
  assert.match(
    source,
    /const runRuntime = \(\) => searchPdfPageTextWithExactPdfiumPackage\(\{\s*bytes,\s*wasmBinary,\s*pageIndex,\s*query,\s*maxMatches\s*\}\)/,
  );
  assert.match(
    source,
    /orchestratePdfTextSearch\(\{[\s\S]*?bytes,[\s\S]*?request,[\s\S]*?availability,[\s\S]*?runtimeBinding,[\s\S]*?runRuntime,[\s\S]*?terminalControl,[\s\S]*?terminateRuntime,?[\s\S]*?\}\)/,
  );
});
...
test('top-level proxy options fail without executing proxy traps', async () => {
  ...
  await assert.rejects(
    searchPdfPageTextWithExactPackageSupervisedSemantics(proxy),
...
test('top-level accessor, symbol, extra-key, and custom-prototype options fail before terminal subscription', async () => {
  ...
  for (const value of cases) {
    await assert.rejects(
      searchPdfPageTextWithExactPackageSupervisedSemantics(value),
      /qualified own data fields/,
    );
  }
...
test('missing wasmBinary, pageIndex, query, and maxMatches keys fail before terminal subscription', async () => {
  for (const key of ["wasmBinary", "pageIndex", "query", "maxMatches"]) {
    const counters = {};
    const options = optionsFor({ terminalControl: quietControl(counters) });
    delete options[key];
    await assert.rejects(
      searchPdfPageTextWithExactPackageSupervisedSemantics(options),
      /qualified own data fields/,
    );
    assert.equal(counters.subscribe || 0, 0);
  }
...
test('wasmBinary accessor fails without invoking caller getter code', async () => {
  ...
  await assert.rejects(
    searchPdfPageTextWithExactPackageSupervisedSemantics(accessor),
    /qualified own data fields/,
  );
  assert.equal(getterCalls, 0);
  assert.equal(counters.subscribe || 0, 0);
...
test('null-prototype top-level options remain accepted with real package execution', async () => {
  ...
  const result = await searchPdfPageTextWithExactPackageSupervisedSemantics(options);
...
test('ordinary-minimal real package execution returns canonical semantic success with unchanged caller bytes', async () => {
  ...
  const result = await searchPdfPageTextWithExactPackageSupervisedSemantics({
    bytes,
    request,
  ...
  assert.equal(result.observations.matchCount, 1);
  assert.deepEqual(result.observations.matches, [{ index: 0, length: 4 }]);
...
for (const [terminalOutcome, errorClass] of [
  ...
  test(`synchronous terminal ${terminalOutcome} wins without PDFium runtime completion and preserves terminal semantics`, async () => {
    ...
    const wasmBefore = Buffer.from(wasmBinary);
    const counters = {};
    const result = await searchPdfPageTextWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('terminal callback mutation of caller WASM fails closed', async () => {
  ...
  await assert.rejects(
    searchPdfPageTextWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('disposer mutation of caller WASM after runtime completion fails closed without aggregation', async () => {
  ...
  const error = await searchPdfPageTextWithExactPackageSupervisedSemantics(optionsFor({
    bytes,
    wasmBinary,
    terminalControl: mutatingControl,
  })).then(
...
test('canonical unavailable and invalid-request semantics remain unchanged through the facade', async () => {
  ...
  const unavailable = await searchPdfPageTextWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
    request,
  ...
  const invalidRequest = await searchPdfPageTextWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
  ...
  assert.equal(invalidRequest.stableError.errorCode, 'invalid_input.pdf_text_search_request');
});
