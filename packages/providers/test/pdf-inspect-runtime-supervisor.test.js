...
} = require('../src/pdf/browser/pdf-inspect-runtime-supervisor');
...
function bindingFor(value = bytes(), overrides = {}) {
  ...
  return {
    ...
    resourceBudgetRef: 'budget:pdf-inspect-v1:supervisor-test',
    ...overrides,
  };
}
...
async function supervise(overrides = {}) {
  ...
  const runRuntime = overrides.runRuntime ?? (async () => ({ openSucceeded: true, pageCount: 2 }));
  const runtimeBinding = overrides.runtimeBinding ?? bindingFor(input);
  const promise = supervisePdfInspectRuntime({
    bytes: input,
    runtimeBinding,
    runRuntime,
...
test('runtime completion wins without termination and returns a frozen raw observation clone', async () => {
  const raw = { openSucceeded: true, pageCount: 3 };
...
for (const terminalOutcome of [
  ...
  test(`${terminalOutcome} produces exact terminal evidence after confirmed termination`, async () => {
    ...
    runtime.resolve({ openSucceeded: true, pageCount: 99 });
...
test('terminal event is snapshotted synchronously before emitter mutation', async () => {
  ...
  assert.equal(state.termination.calls.length, 1);
  runtime.resolve({ openSucceeded: true, pageCount: 1 });
});
...
test('malformed raw runtime observations fail closed', async () => {
  const cases = [
    null,
    {},
    { openSucceeded: true },
    { openSucceeded: true, pageCount: -1 },
    { openSucceeded: true, pageCount: 1, pdfiumLastError: 0 },
    { openSucceeded: false },
  ...
  for (const runtimeBinding of invalidBindings) {
    ...
    await assert.rejects(
      supervisePdfInspectRuntime({
        bytes: input,
        runtimeBinding,
        runRuntime: async () => { executions += 1; return { openSucceeded: true, pageCount: 1 }; },
        terminalControl,
...
test('input and injected objects are not mutated by successful supervision', async () => {
  ...
  const raw = Object.freeze({ openSucceeded: true, pageCount: 4 });
  const terminal = terminalHarness();
  const subscribeBefore = terminal.control.subscribe;
  const result = await supervisePdfInspectRuntime({
    bytes: input,
    runtimeBinding,
  ...
  assert.deepEqual(raw, { openSucceeded: true, pageCount: 4 });
  assert.equal(result.rawObservation.pageCount, 4);
});

test('runtime supervisor source contains no real PDFium, WASM execution, browser/worker or network-loader surface', () => {
  const source = fs.readFileSync(path.join(__dirname, '../src/pdf/browser/pdf-inspect-runtime-supervisor.js'), 'utf8');
