...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `inspect:${fixture.fixtureId}:exact-package-facade`,
    ...
    resourceBudgetRef: 'budget:pdf-inspect-v1:exact-package-facade-test',
...
test('facade module exposes only the bounded inspect function', () => {
  const facade = require('../src/pdf/browser/pdf-inspect-exact-package-facade');
  assert.deepEqual(Reflect.ownKeys(facade), ['inspectPdfWithExactPackageSupervisedSemantics']);
  assert.equal(typeof inspectPdfWithExactPackageSupervisedSemantics, 'function');
  assert.equal(Object.isFrozen(facade), true);
});

test('facade source composes only canonical orchestrator and exact package binding', () => {
  const source = fs.readFileSync(
    path.join(root, 'packages/providers/src/pdf/browser/pdf-inspect-exact-package-facade.js'),
    'utf8',
  );
  assert.match(source, /require\('\.\/pdf-inspect-orchestrator'\)/);
  assert.match(source, /require\('\.\/pdf-inspect-runtime-binding'\)/);
  assert.match(source, /orchestratePdfInspect\(/);
  assert.equal(source.match(/inspectPdfWithExactPdfiumPackage\(/g).length, 1);
...
test('facade binds exactly one runRuntime closure over the same caller bytes and wasm objects', () => {
  const source = fs.readFileSync(
    path.join(root, 'packages/providers/src/pdf/browser/pdf-inspect-exact-package-facade.js'),
    'utf8',
  );
  ...
  assert.match(
    source,
    /const runRuntime = \(\) => inspectPdfWithExactPdfiumPackage\(\{\s*bytes,\s*wasmBinary\s*\}\)/,
  );
  assert.match(
    source,
    /orchestratePdfInspect\(\{[\s\S]*?bytes,[\s\S]*?request,[\s\S]*?availability,[\s\S]*?runtimeBinding,[\s\S]*?runRuntime,[\s\S]*?terminalControl,[\s\S]*?terminateRuntime,?[\s\S]*?\}\)/,
  );
});
...
test('top-level proxy options fail without executing proxy traps', async () => {
  ...
  await assert.rejects(
    inspectPdfWithExactPackageSupervisedSemantics(proxy),
...
test('top-level accessor, symbol, extra-key, and custom-prototype options fail before terminal subscription', async () => {
  ...
  for (const value of cases) {
    await assert.rejects(
      inspectPdfWithExactPackageSupervisedSemantics(value),
      /qualified own data fields/,
    );
  }
...
test('missing wasmBinary key fails before terminal subscription', async () => {
  ...
  await assert.rejects(
    inspectPdfWithExactPackageSupervisedSemantics(options),
    /qualified own data fields/,
  );
  assert.equal(counters.subscribe || 0, 0);
});
...
test('wasmBinary accessor fails without invoking caller getter code', async () => {
  ...
  await assert.rejects(
    inspectPdfWithExactPackageSupervisedSemantics(accessor),
    /qualified own data fields/,
  );
  assert.equal(getterCalls, 0);
  assert.equal(counters.subscribe || 0, 0);
...
test('null-prototype top-level options remain accepted with real package execution', async () => {
  ...
  const result = await inspectPdfWithExactPackageSupervisedSemantics(options);
...
for (const [terminalOutcome, errorClass] of [
  ...
  test(`synchronous terminal ${terminalOutcome} wins without PDFium runtime completion and preserves terminal semantics`, async () => {
    ...
    const wasmBefore = Buffer.from(wasmBinary);
    const counters = {};
    const result = await inspectPdfWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('terminal callback mutation of caller WASM fails closed', async () => {
  ...
  await assert.rejects(
    inspectPdfWithExactPackageSupervisedSemantics(optionsFor({
      bytes,
      wasmBinary,
...
test('disposer mutation of caller WASM after runtime completion fails closed without aggregation', async () => {
  ...
  const error = await inspectPdfWithExactPackageSupervisedSemantics(optionsFor({
    bytes,
    wasmBinary,
    terminalControl: mutatingControl,
  })).then(
...
test('canonical malformed-document semantics remain unchanged through the facade', async () => {
  ...
  assert.equal(result.providerDiagnostics.providerErrorCode, 3);
  assert.equal(result.providerDiagnostics.providerErrorConstant, 'FPDF_ERR_FORMAT');
});
...
test('canonical unavailable and invalid-request semantics remain unchanged through the facade', async () => {
  ...
  const unavailable = await inspectPdfWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
    request,
  ...
  const invalidRequest = await inspectPdfWithExactPackageSupervisedSemantics({
    ...optionsFor(),
    bytes,
  ...
  assert.equal(invalidRequest.stableError.errorCode, 'invalid_input.pdf_inspect_request');
});
