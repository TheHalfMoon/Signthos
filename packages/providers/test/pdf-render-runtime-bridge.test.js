...
const { SUPERVISOR_KIND } = require('../src/pdf/browser/pdf-render-runtime-supervisor');
const { composeSupervisedPdfRenderResult } = require('../src/pdf/browser/pdf-render-runtime-bridge');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `render:${fixture.fixtureId}:bridge`,
    ...
    resourceBudgetRef: 'budget:pdf-render-v1:bridge-test',
...
test('runtime completion maps canonical raw render success into semantic success with pixel digest', () => {
  const bytes = bytesForFixture();
  const request = requestFor(bytes);
  const raw = renderRaw();
  const result = composeSupervisedPdfRenderResult({
    bytes,
    request,
  ...
  assert.deepEqual(result.observations.pixelDigest, {
    algorithm: 'sha256',
    value: crypto.createHash('sha256').update(raw.pixels).digest('hex'),
  });
...
for (const [terminalOutcome, errorClass, errorCode] of [
  [TERMINAL_OUTCOME.CANCELLED, STABLE_ERROR.CANCELLED, 'cancelled.pdf_render_runtime'],
  [TERMINAL_OUTCOME.TIMED_OUT, STABLE_ERROR.TIMEOUT, 'timeout.pdf_render_runtime'],
  [TERMINAL_OUTCOME.RESOURCE_LIMIT_EXCEEDED, STABLE_ERROR.RESOURCE_LIMIT_EXCEEDED, 'resource_limit_exceeded.pdf_render_runtime'],
]) {
...
test('completed path preserves canonical unavailable semantics without render publication', () => {
  const result = compose(completed(renderRaw()), {
...
test('semantic request byte mismatch remains a canonical invalid-input result', () => {
  ...
  const result = composeSupervisedPdfRenderResult({
    ...
    request,
    availability: AVAILABILITY.AVAILABLE,
    supervisedResult: completed(renderRaw()),
  });
  assert.equal(result.outcome, OUTCOME.FAILED);
  assert.equal(result.stableError.errorClass, STABLE_ERROR.INVALID_INPUT);
...
test('bridge requires Buffer bytes at the semantic boundary', () => {
  const bytes = bytesForFixture();
  assert.throws(() => composeSupervisedPdfRenderResult({
...
test('unknown, extra-key, symbol-bearing, and custom-prototype supervisor envelopes fail closed', () => {
  const cases = [
    { kind: 'UNKNOWN', rawObservation: renderRaw() },
    { kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: renderRaw(), extra: true },
    Object.assign({ kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: renderRaw() }, { [Symbol('extra')]: true }),
    Object.assign(Object.create({ inherited: true }), { kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: renderRaw() }),
  ];
...
test('proxy-backed supervisor envelope is rejected without executing proxy traps', () => {
  let traps = 0;
  const proxy = new Proxy(completed(renderRaw()), {
...
test('accessor-backed supervisor kind is rejected without invoking the getter', () => {
  let getterCalls = 0;
  const envelope = { rawObservation: renderRaw() };
...
test('malformed raw render observations fail before the provider can traverse unsafe values', () => {
  let proxyTraps = 0;
  const proxyRaw = new Proxy(renderRaw(), {
    getPrototypeOf() { proxyTraps += 1; throw new Error('trap executed'); },
    ownKeys() { proxyTraps += 1; throw new Error('trap executed'); },
  });

  let getterCalls = 0;
  const accessorRaw = renderRaw();
  ...
  for (const rawObservation of [
    ...
    Object.assign(renderRaw(), { [Symbol('extra')]: true }),
    Object.assign(Object.create({ inherited: true }), renderRaw()),
...
test('bridge does not mutate caller bytes, request, or supervised predecessor evidence', () => {
  const bytes = bytesForFixture();
  const request = requestFor(bytes);
  const rawObservation = renderRaw();
...
test('bridge source contains no runtime, PDFium, browser, timer, network, or dependency execution surface', () => {
  const sourcePath = path.join(root, 'packages/providers/src/pdf/browser/pdf-render-runtime-bridge.js');
  const source = fs.readFileSync(sourcePath, 'utf8');
  for (const forbidden of [
    /renderPdfPageWithLocalWasm/,
    /supervisePdfRenderRuntime\s*\(/,
    /@embedpdf\/pdfium/,
    /PDFiumExt_Init/,
    /\bFPDF_/,
    /\bWebAssembly\b/,
...
test('null-prototype completed envelope and raw observation remain accepted', () => {
  const rawObservation = Object.assign(Object.create(null), renderRaw());
