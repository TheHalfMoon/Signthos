...
const { orchestratePdfRender } = require('../src/pdf/browser/pdf-render-orchestrator');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `render:${fixture.fixtureId}:orchestrator`,
    ...
    resourceBudgetRef: 'budget:pdf-render-v1:orchestrator-render-test',
...
function optionsFor(overrides = {}) {
  ...
  return {
    ...
    runRuntime: overrides.runRuntime ?? (async () => ({ openSucceeded: true, pageIndex: 0, pageCount: 1, width: 1, height: 1, pixelFormat: "BGRA", bytesPerPixel: 4, stride: 4, pixels: Buffer.from([0, 0, 0, 255]) })),
    terminalControl: overrides.terminalControl ?? quietControl(),
    terminateRuntime: overrides.terminateRuntime ?? (async () => ({
      runtimeEvidenceRef: 'runtime-evidence:orchestrator-render-test',
...
test('completed runtime is supervised once and composed into canonical semantic success', async () => {
  const counters = {};
  const result = await orchestratePdfRender(optionsFor({
    runRuntime: async () => {
      counters.runRuntime = (counters.runRuntime || 0) + 1;
      return { openSucceeded: true, pageIndex: 0, pageCount: 1, width: 1, height: 1, pixelFormat: "BGRA", bytesPerPixel: 4, stride: 4, pixels: Buffer.from([0, 0, 0, 255]) };
    },
...
for (const [terminalOutcome, errorClass] of [
  ...
  test(`synchronous terminal ${terminalOutcome} prevents runtime start and composes canonical terminal semantics`, async () => {
    ...
    const result = await orchestratePdfRender(optionsFor({
      ...
      runRuntime: async () => {
        counters.runRuntime = (counters.runRuntime || 0) + 1;
        return { openSucceeded: true, pageIndex: 0, pageCount: 999, width: 1, height: 1, pixelFormat: "BGRA", bytesPerPixel: 4, stride: 4, pixels: Buffer.from([0, 0, 0, 255]) };
      },
...
test('runtime rejection propagates unchanged without semantic substitution', async () => {
  ...
  await assert.rejects(
    orchestratePdfRender(optionsFor({
...
test('malformed runtime completion fails in supervision before semantic composition', async () => {
  await assert.rejects(
    orchestratePdfRender(optionsFor({
      runRuntime: async () => ({ openSucceeded: true, pageIndex: 0, pageCount: -1, width: 1, height: 1, pixelFormat: "BGRA", bytesPerPixel: 4, stride: 4, pixels: Buffer.from([0, 0, 0, 255]) }),
    })),
    /invalid page or dimension fields/,
  );
});
...
test('disposer rejection propagates after runtime completion before semantic composition', async () => {
  ...
  await assert.rejects(
    orchestratePdfRender(optionsFor({ terminalControl })),
...
test('runtime binding mismatch fails before runRuntime is invoked', async () => {
  ...
  await assert.rejects(
    ...
    /do not describe the same PDF render operation/,
  );
...
test('orchestrator requires Buffer bytes before subscribing or starting runtime', async () => {
  ...
  await assert.rejects(
    orchestratePdfRender(optionsFor({
      ...
      terminalControl: quietControl(counters),
      runRuntime: async () => { counters.runRuntime = 1; return { openSucceeded: true, pageIndex: 0, pageCount: 1, width: 1, height: 1, pixelFormat: "BGRA", bytesPerPixel: 4, stride: 4, pixels: Buffer.from([0, 0, 0, 255]) }; },
    })),
...
test('top-level proxy options fail without executing proxy traps', async () => {
  ...
  await assert.rejects(orchestratePdfRender(proxy), /qualified own data fields/);
  assert.equal(traps, 0);
});

test('top-level accessor, symbol, extra-key, and custom-prototype options fail before runtime effects', async () => {
  let runtimeCalls = 0;
  const base = optionsFor({ runRuntime: async () => { runtimeCalls += 1; return { openSucceeded: true, pageIndex: 0, pageCount: 1, width: 1, height: 1, pixelFormat: "BGRA", bytesPerPixel: 4, stride: 4, pixels: Buffer.from([0, 0, 0, 255]) }; } });
  let getterCalls = 0;
  const accessor = { ...base };
  ...
  for (const value of cases) {
    await assert.rejects(orchestratePdfRender(value), /qualified own data fields/);
  }
...
test('null-prototype top-level options remain accepted when all fields are qualified', async () => {
  ...
  const result = await orchestratePdfRender(options);
  assert.equal(result.outcome, OUTCOME.SUCCEEDED);
});
...
test('source imports only supervisor and semantic bridge and contains no direct runtime or external execution surface', () => {
  const source = fs.readFileSync(
    path.join(root, 'packages/providers/src/pdf/browser/pdf-render-orchestrator.js'),
    'utf8',
  );
  assert.match(source, /require\('\.\/pdf-render-runtime-supervisor'\)/);
  assert.match(source, /require\('\.\/pdf-render-runtime-bridge'\)/);
...
test('unsafe semantic request and runtime binding containers fail before runtime effects', async () => {
  ...
  for (const unsafeRequest of unsafeRequests) {
    await assert.rejects(
      orchestratePdfRender(optionsFor({ bytes, request: unsafeRequest, runtimeBinding: binding, runRuntime })),
  ...
  for (const unsafeBinding of unsafeBindings) {
    await assert.rejects(
      orchestratePdfRender(optionsFor({ bytes, request, runtimeBinding: unsafeBinding, runRuntime })),
      /safe own-data objects/,
    );
  }
...
test('unsafe nested digest shapes fail before runtime effects', async () => {
  ...
  for (const badRequest of badRequests) {
    await assert.rejects(
      orchestratePdfRender(optionsFor({ bytes, request: badRequest, runtimeBinding: binding, runRuntime })),
      /do not describe the same PDF render operation/,
    );
  }
  assert.equal(runtimeCalls, 0);
