...
const { SUPERVISOR_KIND } = require('../src/pdf/browser/pdf-text-runtime-supervisor');
const { composeSupervisedPdfTextResult } = require('../src/pdf/browser/pdf-text-runtime-bridge');
...
function requestFor(bytes, overrides = {}) {
  ...
  return {
    operationId: `text:${fixture.fixtureId}:bridge`,
    ...
    resourceBudgetRef: 'budget:pdf-text-v1:bridge-test',
    capabilityParameters: { pageIndex: 0, maxChars: 1000 },
    ...overrides,
  };
}

function textRaw(overrides = {}) {
  return {
    openSucceeded: true,
    pageIndex: 0,
    pageCount: 1,
...
function compose(supervisedResult, overrides = {}) {
  ...
  return composeSupervisedPdfTextResult({
    bytes,
    request,
...
test('runtime completion maps canonical raw text success into semantic success with text digest', () => {
  const bytes = bytesForFixture();
  const request = requestFor(bytes);
  const raw = textRaw();
  const result = composeSupervisedPdfTextResult({
    bytes,
    request,
...
for (const [terminalOutcome, errorClass, errorCode] of [
  [TERMINAL_OUTCOME.CANCELLED, STABLE_ERROR.CANCELLED, 'cancelled.pdf_text_runtime'],
  [TERMINAL_OUTCOME.TIMED_OUT, STABLE_ERROR.TIMEOUT, 'timeout.pdf_text_runtime'],
  [TERMINAL_OUTCOME.RESOURCE_LIMIT_EXCEEDED, STABLE_ERROR.RESOURCE_LIMIT_EXCEEDED, 'resource_limit_exceeded.pdf_text_runtime'],
]) {
...
test('completed path preserves canonical unavailable semantics without text publication', () => {
  const result = compose(completed(textRaw()), {
...
test('semantic request byte mismatch remains a canonical invalid-input result', () => {
  ...
  const result = composeSupervisedPdfTextResult({
    ...
    request,
    availability: AVAILABILITY.AVAILABLE,
    supervisedResult: completed(textRaw()),
  });
  assert.equal(result.outcome, OUTCOME.FAILED);
  assert.equal(result.stableError.errorClass, STABLE_ERROR.INVALID_INPUT);
...
test('bridge requires Buffer bytes at the semantic boundary', () => {
  const bytes = bytesForFixture();
  assert.throws(() => composeSupervisedPdfTextResult({
...
test('unknown, extra-key, symbol-bearing, and custom-prototype supervisor envelopes fail closed', () => {
  const cases = [
    { kind: 'UNKNOWN', rawObservation: textRaw() },
    { kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: textRaw(), extra: true },
    Object.assign({ kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: textRaw() }, { [Symbol('extra')]: true }),
    Object.assign(Object.create({ inherited: true }), { kind: SUPERVISOR_KIND.RUNTIME_COMPLETED, rawObservation: textRaw() }),
  ];
...
test('proxy-backed supervisor envelope is rejected without executing proxy traps', () => {
  let traps = 0;
  const proxy = new Proxy(completed(textRaw()), {
...
test('accessor-backed supervisor kind is rejected without invoking the getter', () => {
  let getterCalls = 0;
  const envelope = { rawObservation: textRaw() };
...
test('malformed raw text observations fail before the provider can traverse unsafe values', () => {
  let proxyTraps = 0;
  const proxyRaw = new Proxy(textRaw(), {
    getPrototypeOf() { proxyTraps += 1; throw new Error('trap executed'); },
    ownKeys() { proxyTraps += 1; throw new Error('trap executed'); },
  });

  let getterCalls = 0;
  const accessorRaw = textRaw();
  ...
  for (const rawObservation of [
    ...
    accessorRaw,
    { ...textRaw(), extra: true },
    Object.assign(textRaw(), { [Symbol('extra')]: true }),
    Object.assign(Object.create({ inherited: true }), textRaw()),
    { ...textRaw(), charCount: -1 },
    { ...textRaw(), truncated: 'yes' },
    { ...textRaw(), text: 123 },
    { ...textRaw(), text: Buffer.from('Signthos') },
...
test('bridge source contains no runtime, PDFium, browser, timer, network, or dependency execution surface', () => {
  const sourcePath = path.join(root, 'packages/providers/src/pdf/browser/pdf-text-runtime-bridge.js');
  const source = fs.readFileSync(sourcePath, 'utf8');
  for (const forbidden of [
    /extractPdfPageTextWithLocalWasm/,
    /supervisePdfTextRuntime\s*\(/,
    /@embedpdf\/pdfium/,
    /PDFiumExt_Init/,
    /\bFPDF_/,
    /\bWebAssembly\b/,
...
test('null-prototype completed envelope and raw observation remain accepted', () => {
  const rawObservation = Object.assign(Object.create(null), textRaw());
