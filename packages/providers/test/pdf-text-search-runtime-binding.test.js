...
} = require('../src/pdf/browser/pdf-text-search-runtime-binding');

const BINDING_OPTIONS_MESSAGE = 'PDF text search exact-package runtime binding options must contain only qualified own data fields';
...
function optionsFor(overrides = {}) {
  return {
    ...
    query: 'query' in overrides ? overrides.query : 'Sign',
    maxMatches: 'maxMatches' in overrides ? overrides.maxMatches : 4,
    ...Object.fromEntries(Object.entries(overrides).filter(([key]) => !['bytes', 'wasmBinary', 'pageIndex', 'query', 'maxMatches'].includes(key))),
  };
}
...
test('exact package runtime binding exposes only the bounded search function', () => {
  const binding = require('../src/pdf/browser/pdf-text-search-runtime-binding');
  assert.deepEqual(Reflect.ownKeys(binding), ['searchPdfPageTextWithExactPdfiumPackage']);
  assert.equal(typeof searchPdfPageTextWithExactPdfiumPackage, 'function');
...
test('binding source imports the exact package initializer without asset or network resolution', () => {
  const sourcePath = path.join(
    __dirname,
    '../src/pdf/browser/pdf-text-search-runtime-binding.js',
  );
  const source = fs.readFileSync(sourcePath, 'utf8');

  assert.match(source, /require\('@embedpdf\/pdfium'\)/);
  assert.match(source, /require\('\.\/pdf-text-search-runtime'\)/);
...
test('binding source carries no provider, supervisor, bridge, orchestrator, or cross-track behavior', () => {
  ...
  for (const forbidden of [
    /require\('\.\/pdf-text-search-provider'\)/,
    /require\('\.\/pdf-text-search-runtime-supervisor'\)/,
    /require\('\.\/pdf-text-search-runtime-bridge'\)/,
    /require\('\.\/pdf-text-search-orchestrator'\)/,
    /extractPdfPageTextWithLocalWasm/,
    /orchestratePdfText\(/,
    /composePdfTextSearchResult/,
    /supervisePdfTextSearchRuntime/,
    /composeSupervisedPdfTextSearchResult/,
...
test('binding rejects malformed envelope shapes before runtime effects', async () => {
  ...
  for (const input of malformed) {
    await assert.rejects(
      searchPdfPageTextWithExactPdfiumPackage(input),
...
test('binding rejects accessor, extra-key, missing-key, and symbol-key envelopes before runtime effects', async () => {
  ...
  const { maxMatches: _dropped, ...withMissingKey } = optionsFor();
...
test('binding preserves canonical byte-view validation without invoking the runtime', async () => {
  ...
  await assert.rejects(
    searchPdfPageTextWithExactPdfiumPackage(optionsFor({ bytes: new Uint8Array(), wasmBinary })),
    /bytes must be a non-empty Uint8Array/,
  );
  await assert.rejects(
    searchPdfPageTextWithExactPdfiumPackage(optionsFor({ bytes, wasmBinary: new Uint8Array() })),
...
test('binding preserves canonical page, query, and match-budget validation without invoking the runtime', async () => {
  for (const pageIndex of [-1, 1.5, '0', null]) {
    await assert.rejects(
      searchPdfPageTextWithExactPdfiumPackage(optionsFor({ pageIndex })),
      /pageIndex must be a non-negative safe integer/,
    );
  }
  for (const query of ['', 123, null, {}, []]) {
    await assert.rejects(
      searchPdfPageTextWithExactPdfiumPackage(optionsFor({ query })),
      /query must be a non-empty string within PDFium int units/,
    );
  }
  for (const maxMatches of [0, -4, 2.5, '4', null]) {
    await assert.rejects(
      searchPdfPageTextWithExactPdfiumPackage(optionsFor({ maxMatches })),
      /maxMatches must be a positive PDFium int/,
    );
  }
});

test('binding delegates package init and raw search extraction for caller-supplied bytes', async () => {
  const bytes = bytesForFixture();
  const wasmBinary = adoptedWasmBytes();
  const bytesBefore = Uint8Array.from(bytes);
  const wasmBefore = Uint8Array.from(wasmBinary);

  const result = await searchPdfPageTextWithExactPdfiumPackage({
    bytes,
    wasmBinary,
    pageIndex: 0,
    query: 'Sign',
  ...
  assert.equal(result.queryLength, 4);
  assert.equal(result.matchCount, 1);
  assert.equal(result.matchesTruncated, false);
  assert.deepEqual(result.matches, [{ index: 0, length: 4 }]);
  assert.deepEqual(bytes, bytesBefore);
  assert.deepEqual(wasmBinary, wasmBefore);
...
test('mismatched caller WASM bytes fail closed without publishing search success', async () => {
  const bytes = bytesForFixture();
