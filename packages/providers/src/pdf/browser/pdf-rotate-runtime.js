...
const QUARTER_TURNS_BY_DEGREES = Object.freeze(new Map([
  [90, 1],
  [180, 2],
  [270, 3],
]));
...
function validateRuntimeSurface(module) {
  ...
  requiredFunction(module, 'FPDFPage_GetRotation');
  requiredFunction(module, 'FPDFPage_SetRotation');
...
function readValidatedOptions(options) {
  ...
  const pageEntry = readOwnData(options, 'pageIndex');
  const degreesEntry = readOwnData(options, 'degreesClockwise');
  if (!bytesEntry || !wasmEntry || !initEntry || !pageEntry || !degreesEntry) {
  ...
  const { data: pageIndex } = pageEntry;
  const { data: degreesClockwise } = degreesEntry;
  ...
  if (!QUARTER_TURNS_BY_DEGREES.has(degreesClockwise)) {
    throw new TypeError('degreesClockwise must be 90, 180, or 270');
  }
  return { bytes, wasmBinary, initPdfium, pageIndex, degreesClockwise };
}

async function rotatePdfPageWithLocalWasm(options) {
  const {
    bytes, wasmBinary, initPdfium, pageIndex, degreesClockwise,
  } = readValidatedOptions(options);
  if (bytes.byteLength > MAX_INPUT_BYTES) {
    throw new Error('PDF input exceeds the rotate resource budget');
  }
  const quarterTurns = QUARTER_TURNS_BY_DEGREES.get(degreesClockwise);
  ...
  try {
    ...
    const rotationBefore = module.FPDFPage_GetRotation(pageHandle);
    if (!Number.isSafeInteger(rotationBefore) || rotationBefore < 0 || rotationBefore > 3) {
      throw new Error('PDFium returned an invalid page rotation');
    }
    const rotationTarget = (rotationBefore + quarterTurns) & 3;
    module.FPDFPage_SetRotation(pageHandle, rotationTarget);
    const rotationAfter = module.FPDFPage_GetRotation(pageHandle);
    if (rotationAfter !== rotationTarget) {
      throw new Error('PDFium rotation did not take effect');
    }
    try {
    ...
    if (writerSize > MAX_OUTPUT_BYTES) {
      throw new Error('PDF output exceeds the rotate resource budget');
    }
  ...
  if (!byteViewEquals(bytes, inputSnapshot)) {
    cleanupErrors.push(new Error('source bytes changed during PDFium rotation'));
  }
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('caller WASM bytes changed during PDFium rotation'));
  }
  if (!byteViewEquals(runtimeWasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('runtime WASM bytes changed during PDFium rotation'));
  }

  if (primaryError && cleanupErrors.length > 0) {
    throw new AggregateError([primaryError, ...cleanupErrors], 'PDFium rotation and cleanup failed');
  }
  if (primaryError) throw primaryError;
  if (cleanupErrors.length > 0) {
    throw new AggregateError(cleanupErrors, 'PDFium cleanup failed');
  }
  if (!result) throw new Error('PDFium rotation produced no result');
  return result;
}

module.exports = Object.freeze({
  rotatePdfPageWithLocalWasm,
  ROTATE_RESOURCE_BUDGETS: Object.freeze({
