...
const { supervisePdfMetadataReadRuntime } = require('./pdf-metadata-runtime-supervisor');
const { composeSupervisedPdfMetadataResult } = require('./pdf-metadata-runtime-bridge');
...
function validateCrossLayerBinding(request, runtimeBinding) {
  ...
  if (ownData(request, 'providerId') !== ownData(runtimeBinding, 'providerId')
    ...
    throw new TypeError('semantic request and runtime binding do not describe the same PDF metadata read operation');
  }
}

async function orchestratePdfMetadataRead(options) {
  if (!exactOwnDataKeys(options, ORCHESTRATOR_INPUT_KEYS)) {
    throw new TypeError('PDF metadata read orchestrator options must contain only qualified own data fields');
  }
  ...
  const supervisedResult = await supervisePdfMetadataReadRuntime({
    bytes,
    runtimeBinding,
    runRuntime,
    terminalControl,
...
module.exports = Object.freeze({ orchestratePdfMetadataRead });
