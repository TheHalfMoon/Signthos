...
async function renderPdfPageWithLocalWasm({
  bytes,
  wasmBinary,
  initPdfium,
  pageIndex,
  ...
  try {
    ...
    if (loadedDocumentHandle === 0) {
      ...
      const width = Math.ceil(pageWidthF);
      const height = Math.ceil(pageHeightF);
      if (!Number.isSafeInteger(width) || width < 1 || !Number.isSafeInteger(height) || height < 1) {
        throw new Error('PDFium page size is not renderable');
      }
      if (width * height > maxPixels) {
        throw new RangeError('PDFium page size exceeds the caller pixel budget');
      }
  ...
  if (!byteViewEquals(bytes, inputSnapshot)) {
    cleanupErrors.push(new Error('source bytes changed during PDFium page render'));
  }
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('caller WASM bytes changed during PDFium page render'));
  }
  if (!byteViewEquals(runtimeWasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('runtime WASM bytes changed during PDFium page render'));
  }

  if (primaryError && cleanupErrors.length > 0) {
    throw new AggregateError([primaryError, ...cleanupErrors], 'PDFium page render and cleanup failed');
  }
  if (primaryError) throw primaryError;
  if (cleanupErrors.length > 0) {
    throw new AggregateError(cleanupErrors, 'PDFium page render cleanup failed');
  }
  if (!result) throw new Error('PDFium page render produced no result');
  return result;
}

module.exports = Object.freeze({ renderPdfPageWithLocalWasm });
