...
const { orchestratePdfInspect } = require('./pdf-inspect-orchestrator');
const { inspectPdfWithExactPdfiumPackage } = require('./pdf-inspect-runtime-binding');
...
async function inspectPdfWithExactPackageSupervisedSemantics(options) {
  if (!exactOwnDataKeys(options, FACADE_INPUT_KEYS)) {
    throw new TypeError('PDF inspect exact-package facade options must contain only qualified own data fields');
  }
  ...
  const runRuntime = () => inspectPdfWithExactPdfiumPackage({ bytes, wasmBinary });

  let result = null;
  let primaryError = null;
  try {
    result = await orchestratePdfInspect({
      bytes,
      request,
      availability,
      runtimeBinding,
  ...
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    ...
    if (primaryError) {
      throw new AggregateError(
        ...
        'PDF inspect exact-package orchestration and WASM integrity check failed',
      );
    }
    throw wasmError;
  }
...
module.exports = Object.freeze({ inspectPdfWithExactPackageSupervisedSemantics });
