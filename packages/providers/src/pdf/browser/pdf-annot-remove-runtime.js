...
function validateRuntimeSurface(module) {
  ...
  requiredFunction(module, 'FPDFPage_RemoveAnnot');
...
function readValidatedOptions(options) {
  ...
  if (!Number.isSafeInteger(entries.annotIndex) || entries.annotIndex < 0) {
    throw new TypeError('annotIndex must be a safe integer >= 0');
  }
  return entries;
}

async function removePdfAnnotWithLocalWasm(options) {
  const {
    bytes, wasmBinary, initPdfium, pageIndex, annotIndex,
  } = readValidatedOptions(options);
  if (bytes.byteLength > MAX_INPUT_BYTES) {
    throw new Error('PDF input exceeds the annot-remove resource budget');
  }
  ...
  try {
    ...
    if (signatureCount !== 0) {
      throw new Error('signed documents are not removal targets');
    }
    ...
    if (annotIndex >= annotCount) {
      throw new Error('annotIndex is out of range for this page');
    }
    ...
    try {
      module.FPDFPage_CloseAnnot(annotHandle);
    } catch (error) {
      throw cleanupFailure('FPDFPage_CloseAnnot', error);
    }
    removedSubtype = subtype;
    const removeStatus = module.FPDFPage_RemoveAnnot(pageHandle, annotIndex);
    if (!removeStatus) {
      throw new Error('PDFium annotation removal did not complete');
    }
    try {
    ...
    if (writerSize > MAX_OUTPUT_BYTES) {
      throw new Error('PDF output exceeds the annot-remove resource budget');
    }
  ...
  if (!byteViewEquals(bytes, inputSnapshot)) {
    cleanupErrors.push(new Error('source bytes changed during PDFium annotation removal'));
  }
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('caller WASM bytes changed during PDFium annotation removal'));
  }
  if (!byteViewEquals(runtimeWasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('runtime WASM bytes changed during PDFium annotation removal'));
  }

  if (primaryError && cleanupErrors.length > 0) {
    throw new AggregateError([primaryError, ...cleanupErrors], 'PDFium annotation removal and cleanup failed');
  }
  if (primaryError) throw primaryError;
  if (cleanupErrors.length > 0) {
    throw new AggregateError(cleanupErrors, 'PDFium cleanup failed');
  }
  if (!result) throw new Error('PDFium annotation removal produced no result');
  return result;
}

module.exports = Object.freeze({
  removePdfAnnotWithLocalWasm,
  ANNOT_REMOVE_RESOURCE_BUDGETS: Object.freeze({
