...
function validateRuntimeBinding(bytes, runtimeBinding) {
  ...
  if (ownData(runtimeBinding, 'providerId') !== PROVIDER_DESCRIPTOR.providerId
    ...
    throw new TypeError('runtimeBinding does not match the qualified PDF_INSPECT_V1 input binding');
  }
...
function validateRawObservation(value) {
  ...
  if (openSucceeded === true) {
    if (!exactOwnDataKeys(value, ['openSucceeded', 'pageCount'])) {
      throw new TypeError('successful raw observation has an invalid shape');
    }
    const pageCount = ownData(value, 'pageCount');
    if (!Number.isSafeInteger(pageCount) || pageCount < 0) {
      throw new TypeError('successful raw observation has an invalid pageCount');
    }
    return Object.freeze({ openSucceeded: true, pageCount });
  }
  if (openSucceeded === false) {
    if (!exactOwnDataKeys(value, ['openSucceeded', 'pdfiumLastError'])) {
      throw new TypeError('rejected raw observation has an invalid shape');
    }
    const pdfiumLastError = ownData(value, 'pdfiumLastError');
    if (!Number.isSafeInteger(pdfiumLastError) || pdfiumLastError < 0) {
      throw new TypeError('rejected raw observation has an invalid pdfiumLastError');
    }
...
function combineFailure(primaryError, cleanupError) {
  if (primaryError && cleanupError) {
    return new AggregateError([primaryError, cleanupError], 'PDF inspect runtime supervision and cleanup failed');
  }
  return primaryError || cleanupError;
}

async function supervisePdfInspectRuntime({
  bytes,
  runtimeBinding,
  runRuntime,
  terminalControl,
