...
function validateRuntimeBinding(bytes, runtimeBinding) {
  ...
  if (ownData(runtimeBinding, 'providerId') !== PROVIDER_DESCRIPTOR.providerId
    ...
    throw new TypeError('runtimeBinding does not match the qualified PDF_TEXT_SEARCH_V1 input binding');
  }
...
function validateRawObservation(value) {
  ...
  if (openSucceeded === true) {
    if (!exactOwnDataKeys(value, SUCCESS_SEARCH_KEYS)) {
      throw new TypeError('successful raw text search observation has an invalid shape');
    }
    ...
    if (!Number.isSafeInteger(pageIndex) || pageIndex < 0
      ...
      throw new TypeError('successful raw text search observation has invalid search fields');
    }
    if (typeof matchesTruncated !== 'boolean') {
      throw new TypeError('successful raw text search observation has invalid search flags');
    }
    if (!Array.isArray(matches) || utilTypes.isProxy(matches) || !matches.every(validMatch)) {
      throw new TypeError('successful raw text search observation has invalid search matches');
    }
    return Object.freeze({
      openSucceeded: true,
      pageIndex,
  ...
  if (openSucceeded === false) {
    if (!exactOwnDataKeys(value, REJECTED_SEARCH_KEYS)) {
      throw new TypeError('rejected raw text search observation has an invalid shape');
    }
    const pdfiumLastError = ownData(value, 'pdfiumLastError');
    if (pdfiumLastError !== PDFIUM_FORMAT_ERROR_CODE) {
      throw new TypeError('rejected raw text search observation has an unqualified pdfiumLastError');
    }
...
function combineFailure(primaryError, cleanupError) {
  if (primaryError && cleanupError) {
    return new AggregateError([primaryError, cleanupError], 'PDF text search runtime supervision and cleanup failed');
  }
  return primaryError || cleanupError;
}

async function supervisePdfTextSearchRuntime({
  bytes,
  runtimeBinding,
  runRuntime,
  terminalControl,
