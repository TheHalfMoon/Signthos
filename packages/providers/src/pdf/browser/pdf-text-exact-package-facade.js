...
const { orchestratePdfText } = require('./pdf-text-orchestrator');
const { extractPdfPageTextWithExactPdfiumPackage } = require('./pdf-text-runtime-binding');
...
async function extractPdfPageTextWithExactPackageSupervisedSemantics(options) {
  if (!exactOwnDataKeys(options, FACADE_INPUT_KEYS)) {
    throw new TypeError('PDF text exact-package facade options must contain only qualified own data fields');
  }
  ...
  const runRuntime = () => extractPdfPageTextWithExactPdfiumPackage({ bytes, wasmBinary, pageIndex, maxChars });

  let result = null;
  let primaryError = null;
  try {
    result = await orchestratePdfText({
      bytes,
      request,
      availability,
      runtimeBinding,
...
module.exports = Object.freeze({ extractPdfPageTextWithExactPackageSupervisedSemantics });
