...
const { supervisePdfTextRuntime } = require('./pdf-text-runtime-supervisor');
const { composeSupervisedPdfTextResult } = require('./pdf-text-runtime-bridge');
...
function validateCrossLayerBinding(request, runtimeBinding) {
  ...
  if (ownData(request, 'providerId') !== ownData(runtimeBinding, 'providerId')
    ...
    throw new TypeError('semantic request and runtime binding do not describe the same PDF text operation');
  }
}

async function orchestratePdfText(options) {
  if (!exactOwnDataKeys(options, ORCHESTRATOR_INPUT_KEYS)) {
    throw new TypeError('PDF text orchestrator options must contain only qualified own data fields');
  }
  ...
  const supervisedResult = await supervisePdfTextRuntime({
    bytes,
    runtimeBinding,
    runRuntime,
    terminalControl,
...
module.exports = Object.freeze({ orchestratePdfText });
