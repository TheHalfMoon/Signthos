...
const { supervisePdfTextSearchRuntime } = require('./pdf-text-search-runtime-supervisor');
const { composeSupervisedPdfTextSearchResult } = require('./pdf-text-search-runtime-bridge');
...
function validateCrossLayerBinding(request, runtimeBinding) {
  ...
  if (ownData(request, 'providerId') !== ownData(runtimeBinding, 'providerId')
    ...
    throw new TypeError('semantic request and runtime binding do not describe the same PDF text search operation');
  }
}

async function orchestratePdfTextSearch(options) {
  if (!exactOwnDataKeys(options, ORCHESTRATOR_INPUT_KEYS)) {
    throw new TypeError('PDF text search orchestrator options must contain only qualified own data fields');
  }
  ...
  const supervisedResult = await supervisePdfTextSearchRuntime({
    bytes,
    runtimeBinding,
    runRuntime,
    terminalControl,
...
module.exports = Object.freeze({ orchestratePdfTextSearch });
