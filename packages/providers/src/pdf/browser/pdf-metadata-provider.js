...
const CAPABILITY_REF = Object.freeze({
  capabilityCode: 'PDF_METADATA_READ_V1',
  capabilityVersion: '1',
});
...
const PROVIDER_CAPABILITY_VERSION = 'signthos.pdf.metadata.read.v1';
...
const TERMINAL_EVIDENCE_SCHEMA = 'signthos.pdf.metadata.read.runtime-terminal.v1';
...
const CAPABILITY_PARAMETER_KEYS = Object.freeze([]);

const ACCEPTED_METADATA_KEYS = Object.freeze(['openSucceeded', 'metadata']);
...
function requestBase(bytes, request) {
  ...
  return deepFreeze({
    ...
    capabilityParameters: Object.freeze({}),
    inputBinding,
    identity,
  });
}
...
function classifyMetadataEvidence(evidence) {
  ...
  if (openSucceeded === true) {
    if (!exactOwnKeys(evidence, ACCEPTED_METADATA_KEYS)) return { kind: 'SHAPE_INVALID' };
    const metadata = ownData(evidence, 'metadata');
    if (!exactOwnKeys(metadata, METADATA_FIELD_KEYS)) return { kind: 'SHAPE_INVALID' };
    const fields = {};
    for (const key of METADATA_FIELD_KEYS) {
      const value = ownData(metadata, key);
      if (!validMetadataValue(value)) return { kind: 'SHAPE_INVALID' };
      fields[key] = value;
    }
    return { kind: 'ACCEPTED', fields };
  }

  if (!exactOwnKeys(evidence, REJECTED_METADATA_KEYS)) return { kind: 'SHAPE_INVALID' };
...
function terminalResult(base, terminal) {
  const semantics = {
    ...
    [TERMINAL_OUTCOME.RESOURCE_LIMIT_EXCEEDED]: {
      ...
      errorCode: 'resource_limit_exceeded.pdf_metadata_read_runtime',
      retryCategory: RETRY_CATEGORY.RETRY_REQUIRES_CHANGED_INPUT_OR_STATE,
    },
...
function composePdfMetadataResult({
  bytes,
  request,
  availability,
  metadataEvidence,
  ...
  if (!base) return invalidRequestFallback(bytes, request, 'invalid_input.pdf_metadata_read_request');
  ...
  if (!exactCapabilityRef(base.capabilityRef)
    ...
    return failed(base, stableError(
      ...
      'unsupported_capability.pdf_metadata_read_v1',
    ));
  }
  ...
  if (terminalOutcomeEvidence !== null && terminalOutcomeEvidence !== undefined) {
    ...
    if (metadataEvidence !== null && metadataEvidence !== undefined) {
      return failed(base, stableError(
        STABLE_ERROR.INVALID_INPUT,
  ...
  if (availability !== AVAILABILITY.AVAILABLE) {
    ...
    return failed(base, stableError(
      ...
      'unavailable.pdf_metadata_read_provider',
      RETRY_CATEGORY.RETRY_MAY_SUCCEED,
    ));
  }

  const metadata = classifyMetadataEvidence(metadataEvidence);
  if (metadata.kind === 'SHAPE_INVALID') {
    return failed(base, stableError(STABLE_ERROR.INVALID_INPUT, 'invalid_input.metadata_evidence_shape'));
  }

  if (metadata.kind === 'ACCEPTED') {
    return deepFreeze({
      ...baseResult(base),
      outcome: OUTCOME.SUCCEEDED,
      observations: {
        title: metadata.fields.title,
        author: metadata.fields.author,
        subject: metadata.fields.subject,
        keywords: metadata.fields.keywords,
        creator: metadata.fields.creator,
        producer: metadata.fields.producer,
        creationDate: metadata.fields.creationDate,
        modDate: metadata.fields.modDate,
        warnings: [],
      },
    });
  }

  if (metadata.kind === 'INPUT_REJECTED') {
    return failed(
      base,
      stableError(
