...
const { selectPdfPageTextWithLocalWasm } = require('./pdf-text-select-runtime');
...
async function selectPdfPageTextWithExactPdfiumPackage(options) {
  if (!exactOwnDataKeys(options, RUNTIME_BINDING_INPUT_KEYS)) {
    throw new TypeError('PDF text selection exact-package runtime binding options must contain only qualified own data fields');
  }
  ...
  return selectPdfPageTextWithLocalWasm({
    bytes,
    wasmBinary,
    initPdfium,
    pageIndex,
...
module.exports = Object.freeze({ selectPdfPageTextWithExactPdfiumPackage });
