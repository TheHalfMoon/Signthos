...
function validateRuntimeSurface(module) {
  ...
  requiredFunction(module, 'FPDFPage_Delete');
...
function readValidatedOptions(options) {
  ...
  if (!bytesEntry || !wasmEntry || !initEntry || !pageEntry) {
  ...
  return { bytes, wasmBinary, initPdfium, pageIndex };
}

async function removePdfPageWithLocalWasm(options) {
  const {
    bytes, wasmBinary, initPdfium, pageIndex,
  } = readValidatedOptions(options);
  if (bytes.byteLength > MAX_INPUT_BYTES) {
    throw new Error('PDF input exceeds the remove resource budget');
  }
  ...
  try {
    ...
    const pageCountBefore = module.FPDF_GetPageCount(documentHandle);
    if (!Number.isSafeInteger(pageCountBefore) || pageCountBefore < 0) {
      throw new Error('PDFium returned an invalid page count');
    }
    if (pageIndex >= pageCountBefore) {
    ...
    module.FPDFPage_Delete(documentHandle, pageIndex);
    const pageCountAfter = module.FPDF_GetPageCount(documentHandle);
    if (pageCountAfter !== pageCountBefore - 1) {
      throw new Error('PDFium page deletion did not take effect');
    }
    ...
    if (writerSize > MAX_OUTPUT_BYTES) {
      throw new Error('PDF output exceeds the remove resource budget');
    }
  ...
  if (!byteViewEquals(bytes, inputSnapshot)) {
    cleanupErrors.push(new Error('source bytes changed during PDFium removal'));
  }
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('caller WASM bytes changed during PDFium removal'));
  }
  if (!byteViewEquals(runtimeWasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('runtime WASM bytes changed during PDFium removal'));
  }

  if (primaryError && cleanupErrors.length > 0) {
    throw new AggregateError([primaryError, ...cleanupErrors], 'PDFium removal and cleanup failed');
  }
  if (primaryError) throw primaryError;
  if (cleanupErrors.length > 0) {
    throw new AggregateError(cleanupErrors, 'PDFium cleanup failed');
  }
  if (!result) throw new Error('PDFium removal produced no result');
  return result;
}

module.exports = Object.freeze({
  removePdfPageWithLocalWasm,
  REMOVE_RESOURCE_BUDGETS: Object.freeze({
