...
function validateRuntimeBinding(bytes, runtimeBinding) {
  ...
  if (ownData(runtimeBinding, 'providerId') !== PROVIDER_DESCRIPTOR.providerId
    ...
    throw new TypeError('runtimeBinding does not match the qualified PDF_TEXT_SELECT_V1 input binding');
  }
...
function validateRawObservation(value) {
  ...
  if (openSucceeded === true) {
    if (!exactOwnDataKeys(value, SUCCESS_SELECT_KEYS)) {
      throw new TypeError('successful raw text selection observation has an invalid shape');
    }
    ...
    if (!Number.isSafeInteger(pageIndex) || pageIndex < 0
      ...
      throw new TypeError('successful raw text selection observation has invalid selection fields');
    }
    if (typeof rectsTruncated !== 'boolean' || typeof unicodeMapError !== 'boolean') {
      throw new TypeError('successful raw text selection observation has invalid selection flags');
    }
    if (typeof text !== 'string') {
      throw new TypeError('successful raw text selection observation has invalid text');
    }
    if (!Array.isArray(rects) || utilTypes.isProxy(rects) || !rects.every(validRect)) {
      throw new TypeError('successful raw text selection observation has invalid selection rects');
    }
    return Object.freeze({
      openSucceeded: true,
      pageIndex,
  ...
  if (openSucceeded === false) {
    if (!exactOwnDataKeys(value, REJECTED_SELECT_KEYS)) {
      throw new TypeError('rejected raw text selection observation has an invalid shape');
    }
    const pdfiumLastError = ownData(value, 'pdfiumLastError');
    if (pdfiumLastError !== PDFIUM_FORMAT_ERROR_CODE) {
      throw new TypeError('rejected raw text selection observation has an unqualified pdfiumLastError');
    }
...
function combineFailure(primaryError, cleanupError) {
  if (primaryError && cleanupError) {
    return new AggregateError([primaryError, cleanupError], 'PDF text selection runtime supervision and cleanup failed');
  }
  return primaryError || cleanupError;
}

async function supervisePdfTextSelectRuntime({
  bytes,
  runtimeBinding,
  runRuntime,
  terminalControl,
