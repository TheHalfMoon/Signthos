...
const { supervisePdfInspectRuntime } = require('./pdf-inspect-runtime-supervisor');
const { composeSupervisedPdfInspectResult } = require('./pdf-inspect-runtime-bridge');
...
function validateCrossLayerBinding(request, runtimeBinding) {
  ...
  if (ownData(request, 'providerId') !== ownData(runtimeBinding, 'providerId')
    ...
    throw new TypeError('semantic request and runtime binding do not describe the same PDF inspect operation');
  }
}

async function orchestratePdfInspect(options) {
  if (!exactOwnDataKeys(options, ORCHESTRATOR_INPUT_KEYS)) {
    throw new TypeError('PDF inspect orchestrator options must contain only qualified own data fields');
  }
  ...
  const supervisedResult = await supervisePdfInspectRuntime({
    bytes,
    runtimeBinding,
    runRuntime,
    terminalControl,
...
module.exports = Object.freeze({ orchestratePdfInspect });
