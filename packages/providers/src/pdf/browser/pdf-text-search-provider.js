...
const CAPABILITY_REF = Object.freeze({
  capabilityCode: 'PDF_TEXT_SEARCH_V1',
  capabilityVersion: '1',
});
...
const PROVIDER_CAPABILITY_VERSION = 'signthos.pdf.text.search.v1';
...
const TERMINAL_EVIDENCE_SCHEMA = 'signthos.pdf.text.search.runtime-terminal.v1';
...
const CAPABILITY_PARAMETER_KEYS = Object.freeze(['pageIndex', 'query', 'maxMatches']);

const ACCEPTED_SEARCH_KEYS = Object.freeze([
  'openSucceeded',
  'pageIndex',
  'pageCount',
  'charCount',
...
function validQuery(value) {
  return typeof value === 'string' && value.length >= 1 && value.length <= MAX_PDFIUM_INT;
}
...
function requestBase(bytes, request) {
  ...
  const query = ownData(capabilityParameters, 'query');
  const maxMatches = ownData(capabilityParameters, 'maxMatches');
  if (!Number.isSafeInteger(pageIndex) || pageIndex < 0) return null;
  if (!validQuery(query)) return null;
  if (!Number.isSafeInteger(maxMatches) || maxMatches < 1 || maxMatches > MAX_PDFIUM_INT) return null;
  ...
  return deepFreeze({
    ...
    capabilityParameters: Object.freeze({ pageIndex, query, maxMatches }),
    inputBinding,
    identity,
  });
}
...
function validMatch(value, charCount) {
  if (!isPlainObject(value) || !exactOwnKeys(value, MATCH_KEYS)) return false;
  const index = ownData(value, 'index');
  const length = ownData(value, 'length');
  if (!Number.isSafeInteger(index) || index < 0) return false;
  if (!Number.isSafeInteger(length) || length < 1 || length > MAX_PDFIUM_INT) return false;
  const matchEnd = index + length;
  return Number.isSafeInteger(matchEnd) && matchEnd <= charCount;
}

function classifySearchEvidence(base, evidence) {
  ...
  if (openSucceeded === true) {
    if (!exactOwnKeys(evidence, ACCEPTED_SEARCH_KEYS)) return { kind: 'SHAPE_INVALID' };
    ...
    const queryLength = ownData(evidence, 'queryLength');
    const matchCount = ownData(evidence, 'matchCount');
    const matchesTruncated = ownData(evidence, 'matchesTruncated');
    const matches = ownData(evidence, 'matches');
    const maxMatches = base.capabilityParameters.maxMatches;
    ...
    if (!Number.isSafeInteger(queryLength) || queryLength < 1 || queryLength > MAX_PDFIUM_INT) {
      return { kind: 'SHAPE_INVALID' };
    }
    if (queryLength !== base.capabilityParameters.query.length) return { kind: 'BINDING_INVALID' };
    if (!Number.isSafeInteger(matchCount) || matchCount < 0) return { kind: 'SHAPE_INVALID' };
    if (typeof matchesTruncated !== 'boolean') return { kind: 'SHAPE_INVALID' };
    if (matchesTruncated !== (matchCount > maxMatches)) return { kind: 'SHAPE_INVALID' };
    if (!Array.isArray(matches) || utilTypes.isProxy(matches)) return { kind: 'SHAPE_INVALID' };
    const expectedMatches = matchesTruncated ? maxMatches : matchCount;
    if (matches.length !== expectedMatches) return { kind: 'SHAPE_INVALID' };
    if (!matches.every((match) => validMatch(match, charCount))) return { kind: 'SHAPE_INVALID' };
    return {
      kind: 'ACCEPTED',
      pageIndex,
      pageCount,
      ...
      matches: matches.map((match) => ({
        index: ownData(match, 'index'),
        length: ownData(match, 'length'),
      })),
    };
  }

  if (!exactOwnKeys(evidence, REJECTED_SEARCH_KEYS)) return { kind: 'SHAPE_INVALID' };
...
function terminalResult(base, terminal) {
  const semantics = {
    ...
    [TERMINAL_OUTCOME.RESOURCE_LIMIT_EXCEEDED]: {
      ...
      errorCode: 'resource_limit_exceeded.pdf_text_search_runtime',
      retryCategory: RETRY_CATEGORY.RETRY_REQUIRES_CHANGED_INPUT_OR_STATE,
    },
...
function composePdfTextSearchResult({
  bytes,
  request,
  availability,
  textEvidence,
  ...
  if (!base) return invalidRequestFallback(bytes, request, 'invalid_input.pdf_text_search_request');
  ...
  if (!exactCapabilityRef(base.capabilityRef)
    ...
    return failed(base, stableError(
      ...
      'unsupported_capability.pdf_text_search_v1',
    ));
  }
  ...
  if (availability !== AVAILABILITY.AVAILABLE) {
    ...
    return failed(base, stableError(
      ...
      'unavailable.pdf_text_search_provider',
      RETRY_CATEGORY.RETRY_MAY_SUCCEED,
    ));
  }

  const search = classifySearchEvidence(base, textEvidence);
  if (search.kind === 'BINDING_INVALID') {
    return failed(base, stableError(STABLE_ERROR.INVALID_INPUT, 'invalid_input.text_search_evidence_binding'));
  }
  if (search.kind === 'SHAPE_INVALID') {
    return failed(base, stableError(STABLE_ERROR.INVALID_INPUT, 'invalid_input.text_search_evidence_shape'));
  }
