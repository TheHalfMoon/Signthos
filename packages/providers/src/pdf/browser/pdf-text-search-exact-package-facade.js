...
const { orchestratePdfTextSearch } = require('./pdf-text-search-orchestrator');
const { searchPdfPageTextWithExactPdfiumPackage } = require('./pdf-text-search-runtime-binding');
...
async function searchPdfPageTextWithExactPackageSupervisedSemantics(options) {
  if (!exactOwnDataKeys(options, FACADE_INPUT_KEYS)) {
    throw new TypeError('PDF text search exact-package facade options must contain only qualified own data fields');
  }
  ...
  const runRuntime = () => searchPdfPageTextWithExactPdfiumPackage({ bytes, wasmBinary, pageIndex, query, maxMatches });

  let result = null;
  let primaryError = null;
  try {
    result = await orchestratePdfTextSearch({
      bytes,
      request,
      availability,
      runtimeBinding,
...
module.exports = Object.freeze({ searchPdfPageTextWithExactPackageSupervisedSemantics });
