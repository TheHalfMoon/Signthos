...
function readPageIndices(value) {
...
function readValidatedOptions(options) {
  ...
  const orderEntry = readOwnData(options, 'pageIndices');
  ...
  const pageIndices = readPageIndices(orderEntry.data);
  if (!pageIndices) {
    throw new TypeError('pageIndices must be a non-empty array of non-negative safe integers');
  }
  return { bytes, wasmBinary, initPdfium, pageIndices };
}

function assertExtractableSubset(pageIndices, pageCount) {
...
async function extractPdfPagesWithLocalWasm(options) {
  const {
    bytes, wasmBinary, initPdfium, pageIndices,
  } = readValidatedOptions(options);
  if (bytes.byteLength > MAX_INPUT_BYTES) {
    throw new Error('PDF input exceeds the extract resource budget');
  }
  const frozenIndices = Object.freeze(pageIndices.slice());
  ...
  try {
    ...
    assertExtractableSubset(frozenIndices, pageCount);
    ...
    const indexByteLength = frozenIndices.length * 4;
    ...
    for (let orderIndex = 0; orderIndex < frozenIndices.length; orderIndex += 1) {
      indexView.setInt32(orderIndex * 4, frozenIndices[orderIndex], true);
    }
    const importStatus = module.FPDF_ImportPagesByIndex(
      targetHandle, sourceHandle, indexPointer, frozenIndices.length, 0,
    );
    try {
    ...
    if (importedPageCount !== frozenIndices.length) {
    ...
    if (writerSize > MAX_OUTPUT_BYTES) {
      throw new Error('PDF output exceeds the extract resource budget');
    }
  ...
  if (!byteViewEquals(bytes, inputSnapshot)) {
    cleanupErrors.push(new Error('source bytes changed during PDFium extraction'));
  }
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('caller WASM bytes changed during PDFium extraction'));
  }
  if (!byteViewEquals(runtimeWasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('runtime WASM bytes changed during PDFium extraction'));
  }

  if (primaryError && cleanupErrors.length > 0) {
    throw new AggregateError([primaryError, ...cleanupErrors], 'PDFium extraction and cleanup failed');
  }
  if (primaryError) throw primaryError;
  if (cleanupErrors.length > 0) {
    throw new AggregateError(cleanupErrors, 'PDFium cleanup failed');
  }
  if (!result) throw new Error('PDFium extraction produced no result');
  return result;
}

module.exports = Object.freeze({
  extractPdfPagesWithLocalWasm,
  EXTRACT_RESOURCE_BUDGETS: Object.freeze({
