...
async function inspectPdfFormWithLocalWasm(options) {
  const { bytes, wasmBinary, initPdfium } = readValidatedOptions(options);
  if (bytes.byteLength > MAX_INPUT_BYTES) {
    throw new Error('PDF input exceeds the form-inspect resource budget');
  }
  ...
  try {
    ...
    if (!Number.isSafeInteger(formType) || formType < 0) {
      throw new Error('PDFium returned an invalid form type');
    }
    ...
    for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
      ...
      for (let annotIndex = 0; annotIndex < annotCount; annotIndex += 1) {
        ...
        if (!Number.isSafeInteger(subtype) || subtype < 0) {
          throw new Error('PDFium returned an invalid annotation subtype');
        }
      ...
      pages.push(Object.freeze({ pageIndex, annotCount, subtypes: Object.freeze(subtypes) }));
    }
    ...
    result = Object.freeze({
      ...
      formType,
      formKind: formType === FORMTYPE_NONE ? 'none' : formType === FORMTYPE_ACRO_FORM ? 'acroform' : 'other',
  ...
  if (!byteViewEquals(bytes, inputSnapshot)) {
    cleanupErrors.push(new Error('source bytes changed during PDFium form inspection'));
  }
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('caller WASM bytes changed during PDFium form inspection'));
  }
  if (!byteViewEquals(runtimeWasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('runtime WASM bytes changed during PDFium form inspection'));
  }

  if (primaryError && cleanupErrors.length > 0) {
    throw new AggregateError([primaryError, ...cleanupErrors], 'PDFium form inspection and cleanup failed');
  }
  if (primaryError) throw primaryError;
  if (cleanupErrors.length > 0) {
    throw new AggregateError(cleanupErrors, 'PDFium cleanup failed');
  }
  if (!result) throw new Error('PDFium form inspection produced no result');
  return result;
}
