...
const { orchestratePdfRender } = require('./pdf-render-orchestrator');
const { renderPdfPageWithExactPdfiumPackage } = require('./pdf-render-runtime-binding');
...
async function renderPdfPageWithExactPackageSupervisedSemantics(options) {
  if (!exactOwnDataKeys(options, FACADE_INPUT_KEYS)) {
    throw new TypeError('PDF render exact-package facade options must contain only qualified own data fields');
  }
  ...
  const runRuntime = () => renderPdfPageWithExactPdfiumPackage({ bytes, wasmBinary, pageIndex, maxPixels });

  let result = null;
  let primaryError = null;
  try {
    result = await orchestratePdfRender({
      bytes,
      request,
      availability,
      runtimeBinding,
  ...
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    ...
    if (primaryError) {
      throw new AggregateError(
        ...
        'PDF render exact-package orchestration and WASM integrity check failed',
      );
    }
    throw wasmError;
  }
...
module.exports = Object.freeze({ renderPdfPageWithExactPackageSupervisedSemantics });
