...
const { orchestratePdfTextSelect } = require('./pdf-text-select-orchestrator');
const { selectPdfPageTextWithExactPdfiumPackage } = require('./pdf-text-select-runtime-binding');
...
async function selectPdfPageTextWithExactPackageSupervisedSemantics(options) {
  if (!exactOwnDataKeys(options, FACADE_INPUT_KEYS)) {
    throw new TypeError('PDF text selection exact-package facade options must contain only qualified own data fields');
  }
  ...
  const startIndex = ownData(options, 'startIndex');
  const selectCount = ownData(options, 'selectCount');
  const maxRects = ownData(options, 'maxRects');
  ...
  const runRuntime = () => selectPdfPageTextWithExactPdfiumPackage({ bytes, wasmBinary, pageIndex, startIndex, selectCount, maxRects });

  let result = null;
  let primaryError = null;
  try {
    result = await orchestratePdfTextSelect({
      bytes,
      request,
      availability,
      runtimeBinding,
  ...
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    ...
    if (primaryError) {
      throw new AggregateError(
        ...
        'PDF text exact-package orchestration and WASM integrity check failed',
      );
    }
    throw wasmError;
  }
...
module.exports = Object.freeze({ selectPdfPageTextWithExactPackageSupervisedSemantics });
