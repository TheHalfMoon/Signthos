...
const { supervisePdfTextSelectRuntime } = require('./pdf-text-select-runtime-supervisor');
const { composeSupervisedPdfTextSelectResult } = require('./pdf-text-select-runtime-bridge');
...
function validateCrossLayerBinding(request, runtimeBinding) {
  ...
  if (ownData(request, 'providerId') !== ownData(runtimeBinding, 'providerId')
    ...
    throw new TypeError('semantic request and runtime binding do not describe the same PDF text selection operation');
  }
}

async function orchestratePdfTextSelect(options) {
  if (!exactOwnDataKeys(options, ORCHESTRATOR_INPUT_KEYS)) {
    throw new TypeError('PDF text selection orchestrator options must contain only qualified own data fields');
  }
  ...
  const supervisedResult = await supervisePdfTextSelectRuntime({
    bytes,
    runtimeBinding,
    runRuntime,
    terminalControl,
...
module.exports = Object.freeze({ orchestratePdfTextSelect });
