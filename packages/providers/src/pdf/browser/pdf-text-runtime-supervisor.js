...
function validateRuntimeBinding(bytes, runtimeBinding) {
  ...
  if (ownData(runtimeBinding, 'providerId') !== PROVIDER_DESCRIPTOR.providerId
    ...
    throw new TypeError('runtimeBinding does not match the qualified PDF_TEXT_V1 input binding');
  }
...
function validateRawObservation(value) {
  ...
  if (openSucceeded === true) {
    if (!exactOwnDataKeys(value, SUCCESS_TEXT_KEYS)) {
      throw new TypeError('successful raw text observation has an invalid shape');
    }
    ...
    if (!Number.isSafeInteger(pageIndex) || pageIndex < 0
      ...
      throw new TypeError('successful raw text observation has invalid page or character fields');
    }
    if (typeof truncated !== 'boolean' || typeof unicodeMapError !== 'boolean'
        || (truncated === true && charCount < 1)) {
      throw new TypeError('successful raw text observation has invalid text flags');
    }
    if (typeof text !== 'string') {
      throw new TypeError('successful raw text observation has invalid text');
    }
    return Object.freeze({
      openSucceeded: true,
      pageIndex,
  ...
  if (openSucceeded === false) {
    if (!exactOwnDataKeys(value, REJECTED_TEXT_KEYS)) {
      throw new TypeError('rejected raw text observation has an invalid shape');
    }
    const pdfiumLastError = ownData(value, 'pdfiumLastError');
    if (pdfiumLastError !== PDFIUM_FORMAT_ERROR_CODE) {
      throw new TypeError('rejected raw text observation has an unqualified pdfiumLastError');
    }
...
function combineFailure(primaryError, cleanupError) {
  if (primaryError && cleanupError) {
    return new AggregateError([primaryError, cleanupError], 'PDF text runtime supervision and cleanup failed');
  }
  return primaryError || cleanupError;
}

async function supervisePdfTextRuntime({
  bytes,
  runtimeBinding,
  runRuntime,
  terminalControl,
