...
function validateRuntimeSurface(module) {
  ...
  requiredFunction(module, 'FPDFPageObj_SetFillColor');
...
async function placePdfMarkWithLocalWasm(options) {
  const {
  ...
  if (bytes.byteLength > MAX_INPUT_BYTES) {
    throw new Error('PDF input exceeds the mark resource budget');
  }
  ...
  try {
    ...
    const fillColorStatus = module.FPDFPageObj_SetFillColor(textObjectHandle, red, green, blue, alpha);
    if (!fillColorStatus) {
      throw new Error('PDFium mark color was not accepted');
    }
    const radians = (angleDegrees * Math.PI) / 180;
    const cosine = Math.cos(radians);
    const sine = Math.sin(radians);
    module.FPDFPageObj_Transform(textObjectHandle, cosine, sine, -sine, cosine, x, y);
    ...
    if (writerSize > MAX_OUTPUT_BYTES) {
      throw new Error('PDF output exceeds the mark resource budget');
    }
    ...
    result = Object.freeze({
      ...
      pageIndex,
      placedText: text,
      placement: Object.freeze({ x, y, fontSize, angleDegrees }),
      color: Object.freeze({ red, green, blue, alpha }),
      font: PLACEMENT_FONT,
      pageCount,
      signatureCount,
  ...
  if (!byteViewEquals(bytes, inputSnapshot)) {
    cleanupErrors.push(new Error('source bytes changed during PDFium mark placement'));
  }
  if (!byteViewEquals(wasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('caller WASM bytes changed during PDFium mark placement'));
  }
  if (!byteViewEquals(runtimeWasmBinary, wasmSnapshot)) {
    cleanupErrors.push(new Error('runtime WASM bytes changed during PDFium mark placement'));
  }

  if (primaryError && cleanupErrors.length > 0) {
    throw new AggregateError([primaryError, ...cleanupErrors], 'PDFium mark placement and cleanup failed');
  }
  if (primaryError) throw primaryError;
  if (cleanupErrors.length > 0) {
    throw new AggregateError(cleanupErrors, 'PDFium cleanup failed');
  }
  if (!result) throw new Error('PDFium mark placement produced no result');
  return result;
}

module.exports = Object.freeze({
  placePdfMarkWithLocalWasm,
  MARK_RESOURCE_BUDGETS: Object.freeze({
